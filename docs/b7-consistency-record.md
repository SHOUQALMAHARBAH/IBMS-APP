...
| the holiday calendar | 2, 3 | NEW, found by that split. The calendar every business-day deadline is counted against is EMPTY on both databases, nothing seeds it, and its only writer has no web caller — so an office cannot enter Eid. The screen shows deadlines as if they were exact; the underlying utility is honest in a comment that they are a lower bound. Rule 2, because the screen answers with a date it cannot support. | OPEN — § 1.57; item 1 needs a source (the gazette), not an invention |
