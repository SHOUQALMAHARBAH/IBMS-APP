 ...
 * `languagePreference` defaults to `'AR'` (the schema's own default and
 * the 3 pre-existing call sites' implicit assumption) — optional and
 * additive, so none of them need updating. */
async function buildOpportunity(
  ownerUserId: string,
  tag: string,
  languagePreference: 'AR' | 'EN' = 'AR',
): Promise<{
...
async function opportunityWithSentRecommendation(
  ...
  tag: string,
  languagePreference: 'AR' | 'EN' = 'AR',
): Promise<{ opportunityId: string; insurerId: string; customerId: string }> {
  const { opportunityId, rfqId, insurerId, customerId } =
    await buildOpportunity(ownerUserId, tag, languagePreference);
  ...
  await request(app.getHttpServer())
    ...
    .set(bearer(token))
    .expect(201);
  return { opportunityId, insurerId, customerId };
}
...
async function acceptedOpportunity(
  ...
  const { opportunityId, insurerId, customerId } =
    await opportunityWithSentRecommendation(
      app,
      token,
      ownerUserId,
      tag,
...
describe('Policy Placement & Issuance (e2e) — backlog Part C #18-19', () => {
  ...
  it('generates a bilingual policy-schedule-summary PDF only once the policy is issued, respecting policy visibility, defaulting to the customer language, and honoring an explicit override', async () => {
    const app = await boot();
    ...
    // DATA_PROTECTION_OFFICER holds no policy.read grant at all (unlike
    // CLAIMS_OFFICER, which this file's policy.read permission DOES
    // grant — see packages/db/prisma/seed-data/permissions.ts).
    ...
    // A placed-but-not-yet-issued policy has no coverage schedule yet —
    // the document endpoint 422s (a data-availability gate, not a
    // visibility one).
    ...
    const enPolicyId = await issuedPolicy(
      app,
      placement.accessToken,
      owningSales.userId,
      'doc-en',
    ...
    // unknown policy -> 404
    await request(app.getHttpServer())
      .get('/policies/11111111-1111-4111-8111-111111111111/document')
      .set(bearer(placement.accessToken))
      .expect(404);

    // a Sales Officer who does NOT own the customer -> 404, even though
    // they hold policy.read — visibility, not just the flat permission,
    // gates this (the whole point of routing through
    // PolicyService.getByIdWithCustomer instead of the repository).
    await request(app.getHttpServer())
      .get(`/policies/${arPolicyId}/document`)
    ...
    const enDefault = await request(app.getHttpServer())
      .get(`/policies/${enPolicyId}/document`)
      .set(bearer(owningSales.accessToken))
      .expect(200);
    ...
    const overridden = await request(app.getHttpServer())
      .get(`/policies/${enPolicyId}/document?language=AR`)
      .set(bearer(owningSales.accessToken))
      .expect(200);
    ...
    const dual = await request(app.getHttpServer())
      .get(`/policies/${arPolicyId}/document?language=DUAL`)
      .set(bearer(owningSales.accessToken))
      .expect(200);
    ...
    await request(app.getHttpServer())
      .get(`/policies/${arPolicyId}/document?language=FR`)
      .set(bearer(owningSales.accessToken))
      .expect(400);
  }, 45000);
