...
export class SlaPolicyRepository {
   ...
   * UNIQUE to (organizationId, policyCode). "SLA-DSR-ACCESS" is a code EVERY
   * office will use, so a global unique let the first office to define it lock
   * out all the others. Equivalent while one Organization exists. PHASE 2 must
   * scope this to the resolved organizationId and restore a unique read. */
  findByCode(policyCode: string): Promise<SlaPolicyWithEscalations | null> {
    return this.prisma.client.slaPolicy.findFirst({
      where: { policyCode },
      include: SLA_POLICY_INCLUDE,
    });
  }
