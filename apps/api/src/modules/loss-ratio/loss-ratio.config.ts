...
import { addMoney, formatMoney, quantizeMoney } from '../../common/money.util';
...
export function computeLossRatio(input: LossRatioInput): LossRatioFigures {
  ...
  // addMoney requires at least one value; seed with 0 so an all-null / empty
  // list (a policy with no settled claims) is a clean zero.
  const periodClaims = addMoney(new Prisma.Decimal(0), ...nets);
