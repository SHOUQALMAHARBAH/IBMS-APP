 ...
 */
/**
 * The maker/checker registry MOVED to `src/common/maker-checker-pairs.config.ts` — `src/common` cannot
 * import from `src/modules`, and `maker-checker.util.ts` needs the list to name a remedy in its refusal.
 * Re-exported here so every existing consumer of this module keeps working unchanged.
 */
export {
  MAKER_CHECKER_REGISTRY,
  type MakerCheckerPair,
} from '../../common/maker-checker-pairs.config';
import type { MakerCheckerPair } from '../../common/maker-checker-pairs.config';
