/**
 * How `docs/permission-catalogue-for-descriptions.txt` is SHAPED — the OWNER'S INPUT FILE for the
 * Arabic one-line permission descriptions. The CLI that reads the database and writes the file is
 * `scripts/generate-permission-descriptions-input.ts`; everything decidable without I/O is here.
 *
 * ## WHY THIS SPLIT, AND WHY CI IS WHAT FOUND IT
 *
 * All of this began in `scripts/`, with its spec importing across the repo root. Every local gate
 * passed — typecheck, lint, both suites — and the DOCKER BUILD failed: `turbo prune web --docker`
 * keeps only the workspaces `web` depends on, and root `scripts/` is not one, so
 * `Cannot find module '../../../scripts/generate-permission-descriptions-input'`. CI is the only
 * environment that starts from nothing, which is exactly the property this repo keeps it for.
 *
 * The fix is not a tsconfig exclusion. The pure half belongs next to `permission-matrix.ts`, which it
 * already depends on for the [5-STATE]/[toggle] grouping, and the I/O half stays a script. Now the
 * logic is typechecked and tested with the rest of web, and the script is a thin shell over it.
 *
 ...
 */
import { buildMatrix, codesOfRow } from './permission-matrix';

/** Where the file lives, relative to the repo root. The script writes it; nothing here touches disk. */
export const OUT = 'docs/permission-catalogue-for-descriptions.txt';
