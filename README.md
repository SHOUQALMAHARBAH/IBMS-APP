...
### Part C · Domain E #41–44 — Customer Requests, Complaints, SLA Management & Customer Communication, with these deferrals
...
- **Domains C–H** — Claims **#23–30 are built** (Notification, Registration, Documentation,
  ...
  #44 Customer Communication are built** (`ServiceRequest` / `Complaint` + the
  generic `SlaTimer`; the read-only cross-module SLA dashboard; consent-gated
  `CommunicationLog` sends — see the Domain E table above);
  **#45–46** (Feedback, Retention),
...
**Part C #44 — Customer Communication (Domain E, Process 44)** — **extends the
  ...
  **422**). **No maker/checker** (logging a send is single-actor cross-functional
  work). **`@code-reviewer` not yet run** (migration + Confidential free-text + a
  consent gate + a new `REJECT` audit row → mandatory; pending). **`/brain-gap`
  filed + pushed** (ibms-brain — `customer-service-lifecycle.md` gains a "Customer
  Communication (Process 44)" section; intro → "#41–44 are built"). web: a new
  **"Communications"** screen (`app/(app)/communications/page.tsx` +
  `lib/customer-service/communication-api.ts` + an `AppNav` entry after
  "Complaints"). **Verification:** +34 api unit (`communication.config.spec.ts`
  19 — `evaluateMarketingConsent` all four reasons + most-recent-by-`grantedAt` +
  re-opt-in + `createdAt` fallback/tiebreak, `resolveChannel` / `resolveLanguage`
  derive / disagree / no-recorded-channel, `isCommunicationChannel`,
  `deriveCommunicationView`, the `CREATE` + `REJECT` audit snapshots carry no
  `subject` / `body`; `communication.service.spec.ts` 15 — 404 customer, derive +
  `CREATE` audit (no body/subject), the channel / language / no-channel 422s,
  marketing granted → `consentRecordId`, marketing no-consent → 422 + `REJECT` +
  no `create`, marketing withdrawn → 422, non-marketing skips consent,
  backdated / future `sentAt`, failed audit doesn't break the send,
  `consent-status` + 404, `get` 404 for a non-Process-44 id, `list` filter
  passthrough). api unit **1446** (96 files). New `test/communication.e2e-spec.ts`
  **1/1 isolated** — COMPLIANCE (no perm) → 403; unknown customer → 404; channel
  `SMS` vs recorded `EMAIL` → 422; language `EN` vs recorded `AR` → 422; a
  9+-digit run in `body` → 400; a plain service send (channel + language derived)
  → 201; a marketing send with no consent → 422; `/consent-status` `no_record`;
