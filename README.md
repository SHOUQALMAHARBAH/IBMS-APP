...
### Part C · Domain F #47–57 — Compliance & Risk (complete), with these deferrals
...
| 53-54 | Operational & PI Risk | **new files** in `apps/api/src/modules/compliance-risk/` (`risk-register.{config,service,controller}.ts`, `pi-policy.{config,service,controller}.ts`, `pi-risk-event.{config,service,controller}.ts`) + 3 repositories · **genuinely no migration, no seed change** — `RiskRegisterItem`/`ProfessionalIndemnityPolicy`/`ProfessionalIndemnityRiskEvent` (Part 5/7.1 core schema) all pre-existed; `risk-register.manage` (`[COMPLIANCE_OFFICER, BRANCH_DEPARTMENT_MANAGER]`) / `pi-policy.manage` (`[COMPLIANCE_OFFICER]`) pre-seeded · **`RiskRegisterItem.riskType`** is exactly the five non-PI categories the source names (operational/cyber/financial/compliance/reputational) — PI gets its own two-model deep-dive instead of a sixth generic row; `open -> closed` plain string, the `RetentionCase`/#46 shape, no maker/checker · **`ProfessionalIndemnityPolicy` is deliberately NOT a fixed-id singleton like `BrokerLicense`** — no `issuedAt`/period-start field to define a validity window with, so a renewal is a brand-new row (preserving `claimsHistorySummary` per period) and "current" = the row with the furthest-out `expiresAt`, the SAME definition `PolicyCheckingRepository.findLatestPiPolicyId` (Process 20/54's discrepancy auto-link) already relied on before this process gave that query a proper home in `PiPolicyRepository.findCurrent()` (now provided directly by BOTH `ComplianceRiskModule` and `PolicyModule`, the `BrokerLicenseRepository` shape) · `ProfessionalIndemnityRiskEvent` already had a real writer before this landing (the Process 20 auto-link) — this process gives those rows their first read/list surface plus a manual-log path (`POST /pi-risk-events`, auto-resolving to the current PI policy when `piPolicyId` is omitted) and a `mitigation` endpoint · **`@code-reviewer` (mandatory — a shared repository refactor + a new derived-"current" resolution over money/PII-adjacent data) → CHANGES REQUESTED → resolved: 1 BLOCKER + 1 MAJOR.** **BLOCKER**: `findFirst({orderBy:{expiresAt:'desc'}})` alone has no secondary sort key and a tie is the likely case (any plain-date input normalizes to an exact UTC midnight via `parseCalendarDate`) — fixed with a deterministic `id: 'desc'` secondary key on both `findCurrent()` and `findMany()`. **MAJOR**: the new PI-risk-event audit snapshot re-embedded an auto-logged event's coverage-figure diff verbatim — the SAME content the sibling `PolicyChecking` audit row deliberately excludes (`sensitive-data-handling.md` — "metadata not body") — fixed by redacting `description` for any row with `sourcePolicyCheckingId` set, leaving a manual entry's own narrative untouched · **2 MINORs**: a manual risk event auto-resolving to a lapsed current PI policy now gets a durable signal (`logger.warn` + `linkedPolicyWasLapsedAtLogTime` in the CREATE audit row); the claims-history update's lack of an optimistic-concurrency guard is now an explicit stated trust-level decision, the `BrokerLicense.scopeOfAuthorization` reasoning · **1 deferred MINOR**: none of the three models carry a `DataClassification` field — flagged as a real open gap needing an actual PCMS determination, not resolved by further stretching the `BrokerLicense` analogy (`claimsHistorySummary` and a risk event's coverage detail plausibly name identifiable customers and settlement amounts) · `apps/web/` gains an **"Operational & PI risk"** screen (`app/(app)/operational-pi-risk/page.tsx` — one page, three sections) · **Verification**: +47 api unit → api unit **1752** (123 files, from 1705). New `test/operational-pi-risk.e2e-spec.ts` **4/4** — risk register CRUD + the PI-category exclusion (400); PI policy create/claims-history/current/list; a deliberate tie-on-`expiresAt` regression test (5 repeated `/current` reads all agree, matching `list()`'s `isCurrent` flag); manual PI risk event log (explicit + auto-resolved policy) + mitigation + list filters. `policy.e2e-spec.ts`'s pre-existing discrepancy-to-PI-risk-event auto-link re-verified unaffected by the repository refactor. Full api e2e suite green aside from the 2 documented chronic flakes (rbac confirmed passing cleanly in isolation; up-sell confirmed as a Prisma engine-connection/timeout error, not an assertion failure, unrelated to this diff). New Playwright `operational-pi-risk.spec.ts` 3/3; full Playwright suite **171/171** (from 168). `npm run typecheck`/`lint`/`build` (api + web) OK | the backlog checkbox says "track," not "block" — there is deliberately no automated new-business-blocking gate for lapsed PI cover, unlike #51's explicit BrokerLicense gate; building one would be a separate, deliberate product decision · no `DataClassification` field on any of the three models (see above — a real open gap, not resolved here) · no renewal-history UI beyond the plain list + `isCurrent` flag · `risk-register.manage`/`pi-policy.manage` are role-level (no per-officer queue) · no retention/disposal policy for PI risk data |
| 55 | Incident Management | **also Part D's M09** (Incident & Breach Management) — `pcms-privacy-modules.md`'s own summary/citation for M09 match `IncidentReport`'s doc comment verbatim, so this one build satisfies both backlog references · **new module** `apps/api/src/modules/compliance-risk/` (`incident.{config,service,controller}.ts` + 5 DTOs) + `repositories/incident.repository.ts` · **genuinely almost no migration** — `IncidentReport` (core schema) pre-existed with every field this needs including both maker/checker columns; `WORKFLOW_TRANSITIONS.IncidentReport` (a strictly linear 7-state chain, no branching: `REPORTED -> CONTAINED -> IMPACT_ASSESSED -> CLASSIFIED -> NOTIFIED -> RECOVERED -> CLOSED`) and `SLA_REGISTRY`'s `incident_containment` (4h, critical severity only) / `incident_senior_management_notification` (1h, Material only) also pre-existed — this module is their first real consumer; new migration `20260906120000` (47th) adds ONLY the `IncidentReport_classification_maker_checker_distinct` CHECK constraint · **no seed change** — four permissions pre-seeded: `incident.report` (broad — 8 roles), `incident.contain`, `incident.classify` (DPO **and** Executive Management — the first permission in this codebase's seed grid shared by BOTH sides of a maker/checker pair, rather than two distinct codes), `incident.notify-regulator` · **the shared-permission segregation is enforced one layer deeper**: `classify()` requires `actor.roles.includes('DATA_PROTECTION_OFFICER')`, `coSign()` requires `actor.roles.includes('EXECUTIVE_MANAGEMENT')`, on top of `assertDifferentActors` + the new DB CHECK — a dedicated unit test proves a single user holding both roles is still blocked from self-classify-and-co-sign · **no bespoke scheduler for "senior management notification (job)"** — `classify()` starts the pre-existing 1-hour SLA timer on Material classification, `notifySeniorManagement()` is a manual stamp (confirmed via repo-wide grep: zero real email/SMS/notification-sending infrastructure exists anywhere in this codebase), and the already-running generic `SlaTimerScheduler` (every 15 minutes, backlog A.8 — `IncidentReport` was already in `SLA_DASHBOARD_SENSITIVE_ENTITY_TYPES`) is the "(job)" that catches a missed manual notification via the #43 SLA dashboard · **the Material co-sign gate is re-derived LIVE** (the #16/#51 pattern) in TWO places — `notifyRegulators()` (CLASSIFIED→NOTIFIED, one of #55's three named checkboxes) and `notifyAffectedSubjects()` (M09's own PDPL scope, not one of the three) — both independently re-check `classification === 'MATERIAL' && !seniorManagementCoSignUserId` immediately before their own write · reads are audited (`isSensitiveDataAccess: true`) despite no `DataClassification` field on the model — reasoned closer to `Claim`/`TransactionMonitoringAlert`/DSR than the Confidential-tier no-read-audit precedent · **`@code-reviewer` (mandatory — a new maker/checker pair + new DB CHECK + new workflow entity + breach data) → CHANGES REQUESTED → resolved: 1 MAJOR** (`title` was missing the `NO_FULL_ACCOUNT_NUMBER` guard `description` already had — fixed) **+ several MINORs** (`maker-checker-segregation.md` gained an `IncidentReport` row + a new "shared permission across two roles" exception clause; `ibms-brain/meta/context/incident-management.md` didn't exist at review time — fixed by writing it; `notifyAffectedSubjects()` was missing the co-sign gate `notifyRegulators()` had — fixed for parity; the web page showed the Classify buttons to an Executive-Management-only user and the Co-sign button to a DPO-only user, both an always-403 dead end — fixed by splitting button visibility to the specific role each sub-action needs) **+ a NIT** (the explicit dual-role unit test above) · also caught and fixed twice, unrelated to the review: a literal `*/15 * * * *` cron string inside a `/** */` JSDoc comment prematurely closes the comment block — reworded to "runs every 15 minutes" · `apps/web/` gains an **"Incident Management"** screen (`app/(app)/incidents/page.tsx` — create form + a per-row action pipeline through all 7 states plus co-sign/notify/regulator sub-actions) · **Verification**: +47 api unit (`incident.config.spec.ts` 10, `incident.service.spec.ts` 37) → api unit **1799** (125 files, from 1752). New `test/incident.e2e-spec.ts` 3/3 (a full CRITICAL incident walked through MATERIAL classification/co-sign/senior-management notification/multi-regulator notification; a NON_MATERIAL classification skipping the co-sign gate entirely; permission/list-filter coverage). New Playwright `incidents.spec.ts` 4/4. `npm run typecheck`/`lint`/`build` (api + web) OK | senior-management notification and regulator/affected-subject notification are all manual stamps, not real sends — no email/SMS/paging integration exists anywhere in this codebase · no `DataClassification` field on `IncidentReport` itself (the model pre-dates this process) · `incident.classify`'s shared-permission maker/checker shape is a deliberate exception, not the new default — new pairs should still use two distinct permission codes unless there's a specific reason to share one · `incident.report`/`incident.contain`/`incident.classify`/`incident.notify-regulator` are all role-level (no per-officer queue) · no retention/disposal policy for incident data |
| 56 | Internal Controls (Maker/Checker) | a one-liner backlog item, not a checkbox list: "fully covered in Part A.5, plus a periodic audit report scanning for any possible self-approval cases" · Part A.5's `assertDifferentActors` (`common/maker-checker.util.ts`) + a DB `CHECK` constraint per pair were verified intact — the only new work is the report · **new module** `apps/api/src/modules/internal-controls/` (`internal-controls.{config,service,controller}.ts` + `internal-controls-audit.scheduler.ts`) — the `SlaDashboardModule` shape (aggregates other modules' data, owns none of it) · **`MAKER_CHECKER_REGISTRY`** is a hand-built inventory of all 15 same-table maker/checker pairs this codebase has a DB `CHECK` constraint for, traced across five migrations (`20260826091424` through `20260906120000`) — `KYCRecord`, `PolicyChecking` (placedByUserId), `Refund`, `DisposalBatch`/`DataSharingApproval`/`DataProcessingAgreement` (all three flagged `dormant: true` — M06/M07/M08 aren't built, no writer exists yet, the #48 dormant-classifier precedent: scanned anyway, forward-compatible), `Settlement`, `CommissionLedgerEntry`, `Recommendation`, `AccessRecertificationItem`, `NeedsAssessment` (two constraints, one maker against two checker fields), `Complaint`, `DataSubjectRequest`, `IncidentReport` · **a 16th pair is handled separately, not in the registry**: `PolicyChecking.checkedByUserId` vs the PARENT `Policy.issuedByUserId` — this build resolved an open question `ibms-brain/meta/context/policy-lifecycle.md` had flagged since #20 (should the DB `CHECK` extend to `issuedByUserId` too?) with a firm answer: it structurally CANNOT — a Postgres `CHECK` constraint compares columns on one row of one table, and these two columns live on different tables; this periodic scan is that one pair's ONLY defense-in-depth backstop beyond the app-level `assertDifferentActors` guard, by design, not an oversight · each pair fetches every row with **no `where` filter** (checker-field nullability differs per model — `AccessRecertificationItem.reviewerUserId` is the one exception, NOT NULL — so filtering happens in the pure classify function instead of trusting 15 different Prisma-generated filter types behind one `unknown` cast), capped at `INTERNAL_CONTROLS_SCAN_LIMIT = 20000` with a `truncated` flag · a best-effort `READ` audit row is always written (counts only); a violation (should never occur — see below) is logged at ERROR and persisted as a `CREATE` `InternalControlsViolation` audit row per finding, no new model · `GET /internal-controls/self-approval-audit` (new permission `internal-controls.view` — `[COMPLIANCE_OFFICER, EXECUTIVE_MANAGEMENT, EXTERNAL_AUDITOR]`, 148 → 149 perms, genuinely new rather than pre-seeded ahead of time; seeded as `internal-controls.audit` originally, renamed 2026-09-06 to fix a real `permissions.spec.ts` failure — every EXTERNAL_AUDITOR grant must end in `.read`/`.view`, and this route was always a plain read) + `InternalControlsAuditScheduler` (daily, 10:00 UTC, after the 09:00 transaction-monitoring sweep) both call the identical scan · **a genuine perf fix found mid-build, not a review comment**: the 16 pair-scans were originally sequential (`for...of` + `await`), which timed out a 30-second e2e test against this session's own long-lived, cumulative `db-test` database; switched to `Promise.all`, cutting the same scan to under 12 seconds — a real production-quality concern, not just a test accommodation · **a clean run (zero violations) is the EXPECTED outcome, not a surprise** — every same-table pair is already DB-enforced, so proving genuine end-to-end detection meant deliberately targeting the one pair with no DB backstop: a violation was planted directly via Prisma (bypassing `assertDifferentActors` entirely, which Postgres itself would reject on any of the other 15 pairs), detected by the scan, then restored before the test finished — the #51 restore-before-finishing precedent scaled to one row; every same-table pair's own classification logic is proven correct via pure-function unit tests instead (a mocked row with `maker === checker` is flagged; Postgres would reject that same row for real) · `apps/web/` gains an **"Internal controls"** screen (`app/(app)/internal-controls/page.tsx` — stat cards, a violations table shown only when non-empty, a by-pair breakdown table, a manual "Run audit now" button) · **a `react-hooks/set-state-in-effect` ESLint false positive was hit and worked around**: `void load()` directly in a mount `useEffect` was flagged even though `load`'s only `setState` calls happen after an `await` (the same shape several other pages in this codebase already use without issue); fixed by matching `apps/web/app/(app)/incidents/page.tsx`'s existing `void (async () => { await load(); })();` pattern instead — the root cause was never fully explained by the rule's own diagnostics, only empirically bisected against a known-working file · **Verification**: +21 api unit (`internal-controls.config.spec.ts` 15, `internal-controls.service.spec.ts` 6) → api unit **1820** (127 files, from 1799). New `test/internal-controls.e2e-spec.ts` **2/2** (permission gating + the full 16-pair report shape; a live-planted cross-table violation detected end-to-end and restored). Full api unit suite 1820/1820 confirmed green. New Playwright `internal-controls.spec.ts` 4/4. `npm run typecheck`/`lint`/`build` (api + web) OK | senior-management-style real-time alerting on a finding does not exist — a violation surfaces via the audit log + the next report read/scheduled run, not a push notification · closing the `PolicyChecking`/`Policy` cross-table gap with a real DB-level mechanism (a trigger, or denormalizing `issuedByUserId` onto `PolicyChecking`) is a deliberate, undone follow-up — this scan is the compensating control, not a replacement for one · a new maker/checker pair added to the schema after this process ships needs a `MAKER_CHECKER_REGISTRY` entry added by hand — nothing discovers pairs automatically from the schema · `internal-controls.view` is role-level (no per-officer queue) · does not feed `#57`'s `InternalAuditFinding` — a violation found here does not create one |
...
| 69 | Cybersecurity — `InformationAsset` | the backlog's own annotation: "fully covered by Part A + `IncidentReport` + `InformationAsset`." · **VERIFIED, not taken at face value — the #68 discipline applied again, and this claim was OVERSTATED MORE than #68's case** · Part A's cybersecurity-relevant items (A.1 Auth/Sessions, A.3 Encryption/Key Management, A.4 Immutable Audit Trail, A.9 Data Masking, A.10 Infra/Deployment) are real but each carries its own already-tracked gap (hardware-token MFA, a real KMS/HSM, encryption-at-rest, unwired `assertSecureChannel`/`assertExportAllowed` callers, "scaffolded, not achieved" env separation) — all already listed under § Known gaps, none restated here · `IncidentReport` (#55/M09) is genuinely real — its own doc comment already frames it as a "unified security + personal-data breach workflow" — but carries no dedicated cyber/category field, just free-text `title`/`description` + a `severity` string (`low|medium|high|critical`) · **`InformationAsset` (ISO 27001 Clause 8.1 asset inventory) was the one genuine gap: completely dormant, zero prior application code anywhere in this repo** — the exact "dormant model, first real writer" shape #58-67 repeatedly found · built its foundational CRUD ONLY: create/list(filtered by `assetType`/`classification`)/get/update on the model's own four fields (`name`, `assetType`, `ownerUserId`, `classification`) — the #67 `Vendor` "minimal CRUD only" scope · `assetType` has no DB enum but is validated app-side against the model's own doc-comment 6-value set (`ASSET_TYPES` in `information-asset.config.ts`: `customer_data | policy_data | document_store | backup | integration | other`) · `ownerUserId` is a bare scalar with no Prisma relation (the `Opportunity.createdByUserId` shape), validated against a real `User` via `UserRepository.findById()` in the SERVICE layer (404 if not found) — the #66 `Employee.userId` link-validation precedent · **this is Domain H's FIRST item to break the "seed before code" pattern** — #66/#67 both found their permissions already pre-seeded; `information-asset.manage` (`[SYSTEM_SECURITY_ADMINISTRATOR, COMPLIANCE_OFFICER]`) had NO pre-seeded grant, requiring a genuine `packages/db/prisma/seed-data/permissions.ts` change and a re-seed of both dev and test databases (151 → 152 permissions) · **new module** `apps/api/src/modules/supporting-operations/information-asset.{config,service,controller,module}.ts` + `repositories/information-asset.repository.ts` · `apps/web/` gains an **"Information Assets"** screen (`app/(app)/information-assets/page.tsx` — list+create+inline-rename, the `vendors` page shape) · **Verification**: +9 api unit (`information-asset.service.spec.ts`) → api unit **2023** (150 files, from 2014). New `test/information-asset.e2e-spec.ts` **7/7** — permission gating; a 400 rejecting an `assetType` outside the documented 6-value set; a 400 rejecting a `classification` outside the `DataClassification` enum; a 404 creating an asset owned by a non-existent user; a real create→list(filtered)→get→update walk; a 404 reassigning an asset to a non-existent owner on update; 404s for an unknown asset on both GET and PATCH. Full api unit suite 2023/2023 confirmed green; full 47-file api e2e suite green across 8 foreground sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `information-assets.spec.ts` 4/4; full Playwright suite 222/222 (from 218). `npm run typecheck`/`lint`/`build` (api + web) OK | Part A's own gaps (hardware-token MFA, a real KMS/HSM, encryption-at-rest, unwired masking/export-gate callers, env separation) stay exactly as tracked under § Known gaps, A.1/A.3/A.9/A.10 · `IncidentReport` still has no dedicated cyber/category taxonomy field · no risk-tiering or review-cadence field on `InformationAsset` (mirroring `Vendor`'s own #67→#71 extension path) — not asked for by #69's own text, not built here · #70 Document Management, #71 Vendor Management, #72-73 BCP/DR, and #74 Knowledge Management (the next four entries below) are all now built, closing Domain H |
| 70 | Document Management — `Document` | full version control (previous/next version) + mandatory classification + a deletion lock disabled by default except via a logged privileged override (`deletionOverrideByUserId`); plus enforcing the "highest classification present" rule (`sensitive-data-handling.md` / `PRIV-STD-02` §6.7) when a file/record mixes classification levels · `Document` (Part 4.2, the electronic Insurance File) pre-exists with THREE prior writers (Policy issuance/attach #18-19, Claim documentation #25, Customer onboarding #3-4), each only ever creating a version-1 row — `versionNumber`/`previousVersionId` and `deletionLocked`/`deletionOverrideByUserId` had sat dormant since before this process; `audit-trail.repository.ts`'s own `findDocumentVersionChain` doc comment already said "no application code creates a second version yet" — a READ-side capability (#57's External Auditor lens) had existed for LONGER than any writer · **version control mirrors `QuotationService.revise` exactly**: `id` must be the chain's own current (leaf) version — a superseded id 422s rather than silently resolving to the real leaf; a genuine concurrent double-version loses `previousVersionId`'s own `@unique` constraint and 409s (`Prisma.PrismaClientKnownRequestError` code `P2002`) · a new version's `category`/`policyId`/`customerId` are inherited from its predecessor, never accepted from the caller — only `fileName`/`storageRef`/`classification` can change per version, `classification` staying mandatory on every version (not just the first) · **deletion is a single-actor privileged override, NOT the M06 Disposal dual control** — `roles-and-segregation-of-duties.md` requires Department Manager + DPO dual control for the *disposal BATCH* destruction decision (`DisposalBatch`, a separate, still-unbuilt process); #70's own singular field and its pre-seeded single-code permission (`document.delete-override`, `[SYSTEM_SECURITY_ADMINISTRATOR, DATA_PROTECTION_OFFICER]`) both anticipate the narrower, ad hoc, single-document shape instead · **a document can only be deleted from the leaf of its own chain — an APPLICATION-level guard, not a DB one**: `Document_previousVersionId_fkey` is `ON DELETE SET NULL`, not `RESTRICT` — Postgres will not stop a delete that orphans a successor, it silently nulls the successor's `previousVersionId` instead; `hasSuccessor()` is checked before every delete, a documented, deliberate limitation (hardening the FK to `RESTRICT` was considered and deferred) · a document still linked to a `ClaimDocument` also cannot be deleted — here the DB's own `ON DELETE RESTRICT` on `ClaimDocument.documentId` IS a genuine backstop behind the same app-level pre-check · `overrideDeletionLock`/`deleteIfUnlocked` are both status-conditional writes (`updateMany`/`deleteMany` re-asserting `deletionLocked` in the `where`), never a `findUnique` + unconditional write · **the "highest classification present" rollup is scoped to `Document.policyId` only** (`GET /documents/classification-summary?policyId=`) — matching the schema's own "every Policy resolves to ONE electronic Insurance File" framing; a claim-linked document (joined via `ClaimDocument`, never carrying its own `policyId`) is deliberately out of scope · **`#70`'s new `document.manage` routes deliberately do NOT duplicate `#57`'s existing `GET /audit-trail/documents/:id/history`** (`document-history.read`, COMPLIANCE/AUDITOR only, a compliance-forensics lens) — the people who actually upload/version documents mostly cannot reach that route at all; #70's own `GET /documents`/`GET /documents/:id` are a plain operational browse instead · **no new permission, no migration** — `document.manage`/`document.delete-override` were both pre-seeded (Domain H's "seed before code" pattern holding again after #69 broke it) · **new module** `apps/api/src/modules/supporting-operations/document.{config,service,controller,module}.ts` + `repositories/document.repository.ts` · `apps/web/` gains a **"Documents"** screen (`app/(app)/documents/page.tsx` — a policy-scoped lookup, per-row version/unlock/delete actions, a classification-summary lookup) · **Verification**: +21 api unit (`document.config.spec.ts` 6, `document.service.spec.ts` 15) → api unit **2044** (152 files, from 2023). New `test/document.e2e-spec.ts` **6/6** — permission gating (`document.manage` vs `document.delete-override` checked separately); create-version → 422 on a superseded id; the full lock/unlock/delete lifecycle (409 while locked, unlock, a second override 409s, delete succeeds, row confirmed gone via a direct Prisma read since the deleting ADMIN role holds no read permission); 409 deleting a document with a newer version even once unlocked; 409 deleting a document still attached to a claim; the classification-summary rollup + a 404 for an unknown policy. Full api unit suite 2044/2044 confirmed green; full 48-file api e2e suite green across 8 foreground sub-batches — one confirmed-transient environment hiccup and one confirmed-transient TOTP-timing flake this run (both unrelated files, re-confirmed clean on isolated re-run), both documented chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `documents.spec.ts` 5/5; full Playwright suite 227/227 (from 222). `npm run typecheck`/`lint`/`build` (api + web) OK — two real lint catches: unescaped apostrophes (`react/no-unescaped-entities`) in the new web page's prose, and a TypeScript literal-type inference gap in the new Playwright spec's mutable fixture object, both fixed | hardening `Document_previousVersionId_fkey` from `ON DELETE SET NULL` to `RESTRICT` is a real, documented follow-up, not built here · the M06 Disposal Management dual-control workflow (`DisposalBatch`, Department Manager + DPO) remains a separate, still-unbuilt process · `#57`'s External Auditor document-history lens is unchanged, still the compliance-forensics view · a new version's classification is validated against the full `DataClassification` set regardless of whether the chain is customer-linked — `create-customer-document.dto.ts`'s own narrower CONFIDENTIAL/HIGHLY_CONFIDENTIAL-only restriction is a create-time-only rule, not re-derived for later versions · #71 Vendor Management, #72-73 BCP/DR, and #74 Knowledge Management (the next three entries below) are all now built, closing Domain H |
| 71 | Vendor Management — `Vendor`, `DataProcessingAgreement` (M07) | risk tiering (low/medium/high) before any data share or access grant; a mandatory Data Processing Agreement for Medium/High tier + additional DPO approval for High tier before the first share; mandatory annual review + confirmation of data return/destruction on termination + access revocation within 2 business days · #67 (Procurement) built the foundational `Vendor` CRUD and explicitly deferred `riskTier`/`annualReviewDueAt`/`terminationDataReturnConfirmedAt`/`accessRevokedAt` to this process — SAME model, SAME module, extended not duplicated · **checkbox 1 has no live call site to enforce against**: `DataSharingApproval` (M08) has zero prior writer and no model represents a generic "access grant" at all — built `computeDataShareReadiness()` (`vendor.config.ts`) as a pure, queryable rule (`GET /vendors/:id/data-share-readiness`) instead of inventing the M08 workflow, an honest gate with NO live enforcement call site yet · **`DataProcessingAgreement`'s maker/checker pair (`assessedByUserId`/`dpoApprovedByUserId`) already had a DB `CHECK` constraint and a covered-pairs table entry since the A.5 foundational work, zero prior writer** — this process is that first real writer; `dpa.approve` (DPO only) was already pre-seeded as a DISTINCT permission code from `vendor.manage`, the maker-checker default · **not the M06 Disposal dual control** — the Department-Manager-+-DPO dual control the segregation-of-duties lex files describe is the M06 `DisposalBatch` BATCH-destruction workflow (a separate, still-unbuilt process), not this process's own, already-distinct DPA maker/checker pair · **`vendor_termination_access_revocation` (2 business days) needed a genuinely NEW SLA registry entry** — unlike this SAME process's own `vendor_annual_review` (already pre-seeded, zero prior caller), `pdpl-sla-timers.md`'s lex table had NO row at all for vendor termination access revocation; added to both the lex table and `sla-registry.config.ts`, sourced directly from the backlog's own "2 business days" text (the M03 sourcing precedent) · **`Vendor` has no explicit "terminated" status** — termination is `terminationDataReturnConfirmedAt` going null-to-set, gated by a mandatory explicit `confirmDataReturnOrDestruction: true` attestation (the `FulfilDsrDto` staff-attestation precedent); `accessRevokedAt` is a separate, subsequent stamp requiring termination first (422 otherwise), both independently status-conditional (409 on double-execution) · risk tiering auto-schedules the annual-review SLA on the FIRST Medium/High tiering only — a later downgrade does not un-schedule it; `recordAnnualReview()` re-bases +12 months via the documented resolve-then-start-new SLA re-basing shape · **no new permission, no migration** — `vendor.manage`/`dpa.approve` were both pre-seeded · **new files** `apps/api/src/modules/supporting-operations/data-processing-agreement.{service,controller}.ts` + `repositories/data-processing-agreement.repository.ts`, registered in the SAME `vendor.module.ts`; `vendor.{config,service,controller,repository}.ts` extended, not duplicated · `apps/web/` gains a **vendor detail screen** (`app/(app)/vendors/[id]/page.tsx` — risk tiering, readiness check, DPA lifecycle, termination/revocation), the list page gaining a "Manage" link into it · **Verification**: +42 api unit (`vendor.config.spec.ts` 13, `vendor.service.spec.ts` rewritten to 27, `data-processing-agreement.service.spec.ts` 10, plus an `sla-registry.config.spec.ts` count-assertion update) → api unit **2086** (154 files, from 2044). Extended `test/vendor.e2e-spec.ts` to **8/8** (from 4) — permission gating unchanged; a full risk-tiering→annual-review→re-review walk; the full DPA maker/checker lifecycle + readiness gate for a High-tier vendor (403 self-approval, 409 double-approval); Low-tier readiness with no DPA; the termination→revocation walk with order-guarding and double-execution 409s. Full api unit suite 2086/2086 confirmed green; full 48-file api e2e suite green across 8 foreground sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `vendor-detail.spec.ts` 5/5; full Playwright suite 232/232 (from 227). `npm run typecheck`/`lint`/`build` (api + web) OK — one real accessibility catch (an unlabeled risk-tier `<select>`), fixed with a wrapping `<label>` | M08 (`DataSharingApproval`) remains a separate, still-unbuilt PCMS module — `computeDataShareReadiness()` is built for it to eventually call, not a substitute for building it · M06 (`DisposalBatch`) remains a separate, still-unbuilt batch-destruction dual-control workflow · the `vendor_termination_access_revocation` SLA figure has no independent `PRIV-SOP-10` citation beyond the backlog's own "2 business days" text, the same documented-sourcing-gap shape as `CLAIM_LARGE_THRESHOLD_JOD` · #72-73 BCP/DR and #74 Knowledge Management (the next two entries below) are both now built, closing Domain H |
| 72–73 | Business Continuity & Disaster Recovery — `BcpDrPlan` | a plan for every scenario the source names explicitly (system outage, site/office loss, cyberattack/ransomware, key-staff unavailability, insurer-side service interruption); a documented RTO/RPO + last-tested date + next-test-due date per plan · `BcpDrPlan` (schema's own doc comment: "Process 72-73 — continuity/DR plans and tests") pre-exists with zero prior application code — the same "dormant model, first real writer" shape #58-71 repeatedly found · `bcp-dr.manage` (`[SYSTEM_SECURITY_ADMINISTRATOR, COMPLIANCE_OFFICER]`) was already pre-seeded — no seed change needed · **`meta/lex/backup-rpo-rto.md` already covers ONE narrow, already-tested slice of scenario #1 (`system_outage`)** — the database backup/restore drill (`.github/workflows/backup-drill.yml`, weekly, a real pass/fail on row-count parity + a timed RTO check); that lex file's own RPO/RTO figures (24h/15min) are DRAFT, database-specific, and pre-date this process — not restated or duplicated here · the other four scenarios are organizational/procedural plans with NO code-level automation anywhere in this repo — `BcpDrPlan` is a plain record of a plan's existence and test history, not a system that executes or verifies the plan itself · **no sourced test-cadence figure exists for BCP/DR plans generally** — unlike #71's Vendor annual review (an explicit "Annual" cadence already in `pdpl-sla-timers.md`), nothing names how often a plan must be re-tested; `nextTestDueAt` is therefore a plain CALLER-SUPPLIED field on `recordTest()` (`RecordBcpDrPlanTestDto.nextTestDueAt`, mandatory), never auto-computed from a fabricated interval · **this is deliberately NOT wired into `SLA_REGISTRY`** — that registry is for PDPL-sourced SLAs `pdpl-sla-timers.md` tracks; BCP/DR testing cadence is a CBJ operational-resilience concern (Part 10.4/10.5), a different regulatory domain entirely · **checkbox 1 is answered by a genuine coverage/gap check, not just a CRUD a caller must trust was populated correctly**: `GET /bcp-dr-plans/coverage` (`computeScenarioCoverage()`) returns all five named scenarios every time, each with its own `plans` array (possibly empty) and a `hasPlan` flag — the #59/#69 "verify coverage" discipline applied to a five-item checklist instead of a single field · `planDocumentId` (a bare scalar, no Prisma relation) is validated against a real `Document` via `DocumentRepository.findById()` in the service layer (404 if not found) — the #66/#69 link-validation precedent, now reachable because #70 (Document Management) built real `Document` CRUD for this process to validate against · `scenario` is immutable once a plan is created · **new module** `apps/api/src/modules/supporting-operations/bcp-dr-plan.{config,service,controller,module}.ts` + `repositories/bcp-dr-plan.repository.ts` · `apps/web/` gains a **"BCP / DR plans"** screen (`app/(app)/bcp-dr-plans/page.tsx` — the coverage view with gaps flagged in red, a create form, per-plan record-test action) · **Verification**: +16 api unit (`bcp-dr-plan.config.spec.ts` 6, `bcp-dr-plan.service.spec.ts` 10) → api unit **2102** (156 files, from 2086). New `test/bcp-dr-plan.e2e-spec.ts` **6/6** — permission gating; a 400 rejecting a scenario outside the documented 5-value set; a 404 creating a plan with a `planDocumentId` that doesn't reference a real `Document`; a real create→list(filtered)→get→update→record-test walk with a REAL linked `Document` fixture (scenario proven immutable across the update); 404s for an unknown plan on GET/PATCH; the five-scenario coverage view with a real gap-vs-covered assertion. Full api unit suite 2102/2102 confirmed green; full 49-file api e2e suite green across 7 foreground sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `bcp-dr-plans.spec.ts` 5/5; full Playwright suite 237/237 (from 232). `npm run typecheck`/`lint`/`build` (api + web) OK | the database backup/restore drill remains the one piece of real infrastructure automation touching any of the five scenarios — the other four have no code-level backing · any future automated BCP/DR test-cadence SLA would need a real sourced figure (a `PRIV-SOP`/CBJ circular citation) before joining `SLA_REGISTRY`, not invented here · #74 (Knowledge Management, the next entry below) is now built, closing Domain H |
| 74 | Knowledge Management — `KnowledgeBaseArticle` (closes Domain H) | a bilingual knowledge base: product knowledge, insurer appetite, rate guides, regulatory updates · `KnowledgeBaseArticle` (schema's own doc comment: "Process 74 — product knowledge, insurer appetite, rate guides, regulatory updates for staff") pre-exists with zero prior application code — the same "dormant model, first real writer" shape #58-73 repeatedly found, closing Domain H on the same pattern it opened with · `kb.publish` (`[COMPLIANCE_OFFICER, BRANCH_DEPARTMENT_MANAGER, PLACEMENT_TECHNICAL_OFFICER]`) was already pre-seeded — no seed change needed · **"bilingual" here is OPTIONAL-per-article, not mandatory-both** — the sibling `DocumentTemplate` model (Part 11.2, regulator-facing formal documents) has `nameEn`/`nameAr`/`bodyEn`/`bodyAr` ALL `NOT NULL`; `KnowledgeBaseArticle` is the opposite — only `title` is mandatory, `titleAr`/`bodyEn`/`bodyAr` are all nullable, so an article may exist in English only, Arabic only, or both, a deliberate reading of the schema's own nullability · **creation IS publishing** — `publishedAt` defaults to `now()` at the DB level with no nullable "draft" state, matching the pre-seeded permission's own verb (`kb.publish`, not `kb.manage`/`kb.create`) · **`kb.publish` gates the WHOLE surface (create/list/get/update), not just the publish action** (the #67/#69/#71 "one pre-seeded permission gates the whole CRUD" precedent) — with a real, documented consequence: only that permission's own three roles can even READ the knowledge base via this API, since no second, broader read permission was pre-seeded; a genuinely useful company-wide knowledge base would want much wider read access, but this is a real, deliberate scope limit of the backlog's own permission grid, not solved by inventing a permission the seed data doesn't define · `category` is mutable via `PATCH`, unlike #72-73's `BcpDrPlan.scenario` (deliberately immutable) — recategorizing an article is just fixing metadata, not rewriting a scenario's own history; the SAME domain can make different mutability calls for a similarly-shaped "classifying dimension" field depending on what it actually represents · no maker/checker, no SLA timer, no cross-entity FK validation — the simplest Domain H CRUD built this session, comparable to #67 Vendor's original minimal scope · **new module** `apps/api/src/modules/supporting-operations/knowledge-base-article.{config,service,controller,module}.ts` + `repositories/knowledge-base-article.repository.ts` · `apps/web/` gains a **"Knowledge Base"** screen (`app/(app)/knowledge-base/page.tsx` — a bilingual list with RTL-aware Arabic columns/inputs, a publish form, inline title/titleAr edit) · **Verification**: +9 api unit (`knowledge-base-article.service.spec.ts`) → api unit **2111** (157 files, from 2102). New `test/knowledge-base-article.e2e-spec.ts` **5/5** — permission gating; a 400 rejecting a category outside the documented 4-value set; a real create(English-only)→list(filtered)→get→update(add Arabic translation) walk; a fully bilingual create in one call; 404s for an unknown article on both GET and PATCH. Full api unit suite 2111/2111 confirmed green; full 50-file api e2e suite green across 8 foreground sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `knowledge-base.spec.ts` 5/5; full Playwright suite 242/242 (from 237). `npm run typecheck`/`lint`/`build` (api + web) OK | no broader, company-wide read permission for the knowledge base — a real, documented gap, not solved here since the backlog's own pre-seeded grid names only `kb.publish` · `DocumentTemplate` (Part 11.2) itself remains a separate, still-unbuilt model for regulator-facing bilingual formal documents · **Domain H (Supporting Operations, backlog #66-74) is now COMPLETE** |
...
- **Domains C–H** — Claims **#23–30 are built** (Notification, Registration, Documentation,
  ...
  stamp. #53-54 Operational & Professional Indemnity Risk is built: a generic risk
  register over the five non-PI categories the source names, plus the broker's own PI
  policy record (a renewal is a new row, "current" = the furthest-out expiry, NOT a
  fixed-id singleton like #51's `BrokerLicense`) and the PI risk events a Policy
  Checking discrepancy already auto-logs (Process 20). #55 Incident Management is built:
  the full 7-state `IncidentReport` chain, a Material classification's DPO + Executive
  Management co-sign maker/checker pair, the pre-existing generic `SlaTimerScheduler`
  standing in for the "senior management notification (job)" checkbox (no bespoke
  scheduler — no real notification-sending infrastructure exists anywhere in this
  codebase to build one on top of), and independent multi-regulator notification — this
  build is also Part D's M09 (Incident & Breach Management), one build satisfying both
  backlog references. #56 Internal Controls (Maker/Checker) is built: Part A.5's
  `assertDifferentActors` + DB `CHECK` guards were verified intact, plus the new
  periodic self-approval audit report scanning all 15 same-table maker/checker pairs
  and the one cross-table pair (`PolicyChecking`/`Policy`) no single-table `CHECK`
  constraint can express. #57 Internal Audit is built — **closes Domain F, #47–57 all
  built or verified-covered**: `InternalAuditFinding`'s log/remediate/close tracker (two
  ...
  in this codebase. #61 Employee Performance is also built: new clients/premium/
  commission/renewal rate/cross-sell rate, resolving each `Employee` to its linked
  `User` account (both `Employee` itself and `User.employeeId` are otherwise dormant,
  since Domain H/#66 HR isn't built) before computing anything. #62 Portfolio Analysis
  is also built: a query by line/insurer/client segment/geography — unlike #59–61, no
  model, migration, scheduler, or new permission at all, the lightest Domain G process
  so far; a pure on-demand read splitting cleanly between two real DB-side `groupBy`
  calls (line/insurer, live on `Policy` itself) and two capped `findMany`-plus-reduce
  breakdowns (client segment/geography, both crossing a relation Prisma's `groupBy`
  cannot follow). #63 Profitability Analysis is also built: commission income vs.
  cost-to-serve per segment/line — REUSES (not duplicates) the same written-policy
  read #40's `FinancialReportService` already loads, via a shared
  `ProfitabilityPolicyRepository` provided independently in both `FinanceModule` and
  the new `ProfitabilityAnalysisModule`'s own `providers` arrays, no cross-module
  `imports`/`exports` wiring; `profitability-analysis.view` grants Executive
  Management / Finance only — the first Domain G permission NOT also granted to
  Branch/Department Manager. #65 Strategic Planning Inputs is also built —
  **the last Domain G item built; #64 Executive Management Reporting deliberately
  not built yet** — exporting portfolio (#62) + market/insurer-performance (#60)
  data for planning cycles: no new model, and the SAME "reuse the repository, never
  the service" rule extended to TWO shared repositories at once
  (`PortfolioAnalysisRepository` + `InsurerPerformanceRepository`), plus the first
  real writer of the dormant `AuditAction.EXPORT` (activating Part 10.3's bulk-export
  ...
  module. #68 Internal Information Technology is VERIFIED, not built — the
  backlog's own annotation claims it's already covered by A.10 (environment
  separation) and "Part 10.5" (change management with security sign-off), but
  that claim is only PARTIALLY accurate: no standalone data table is genuinely
  needed (correct), but A.10's Dev/Test/UAT/Prod separation is this SAME
  ...
  unlike #66/#67). #70 Document Management is built too: `Document` (Part
  4.2) had three prior writers (Policy/Claim/Customer attach flows) that
  each only ever created a version-1 row — `versionNumber`/
  `previousVersionId` and `deletionLocked`/`deletionOverrideByUserId` sat
  dormant since before this process, with a READ-side capability (#57's
  audit-trail version-chain walker) existing LONGER than any writer.
  Version-chain creation mirrors `QuotationService.revise`; deletion is a
  single-actor privileged override (NOT the M06 Disposal dual control,
  a separate unbuilt process); the "highest classification present" rollup
  (`PRIV-STD-02` §6.7) is scoped to a Policy's own documents. No new
  permission, no migration — both were pre-seeded, the "seed before code"
  pattern holding again after #69 broke it. #71 Vendor Management (M07) is
  also built: #67 (Procurement) explicitly deferred `riskTier`/
  `annualReviewDueAt`/`terminationDataReturnConfirmedAt`/`accessRevokedAt`
  to this process, extending the SAME `Vendor` model/module rather than a
  parallel one. `DataProcessingAgreement`'s maker/checker pair
  (`assessedByUserId`/`dpoApprovedByUserId`) already had a DB `CHECK`
  constraint since the A.5 work with zero prior writer — this process is
  that first real writer, gated by `dpa.approve` (DPO only), a permission
  code already DISTINCT from `vendor.manage`. The backlog's own "risk
  tiering before any data share or access grant" checkbox has no live
  action to enforce against (M08's `DataSharingApproval` is unbuilt, and no
  generic "access grant" model exists) — built as a pure, queryable
  `computeDataShareReadiness()` gate instead of inventing M08. A genuinely
  NEW SLA registry entry (`vendor_termination_access_revocation`, 2
  business days) was needed — `pdpl-sla-timers.md` had no row for it at
  all, unlike this same process's own already-pre-seeded
  `vendor_annual_review`. **#72-73 Business Continuity & Disaster
  Recovery is also built**: `BcpDrPlan` pre-existed with zero prior
  application code; `meta/lex/backup-rpo-rto.md` already covers ONE
  narrow, already-tested slice of scenario #1 (`system_outage`) via the
  weekly database restore drill, whose own draft RPO/RTO figures are not
  restated here — the other four named scenarios have no code-level
  automation anywhere in this repo. No sourced BCP/DR test-cadence figure
  exists, so `nextTestDueAt` is caller-supplied and this process is
  deliberately NOT wired into `SLA_REGISTRY` (a CBJ operational-resilience
  concern, a different regulatory domain from the PDPL-sourced SLAs that
  registry tracks). Checkbox 1 ("a plan for every named scenario") is
  answered by a real five-scenario coverage/gap-check endpoint. #74
  Knowledge Management is also built, closing Domain H: `KnowledgeBaseArticle`
  was another dormant model with zero prior application code; unlike
  `DocumentTemplate` (Part 11.2), which requires BOTH `nameEn`/`nameAr`/
  `bodyEn`/`bodyAr`, this model's bilingual fields are all optional except
  `title` — an article may exist in English only, Arabic only, or both.
  Creation IS publishing (`publishedAt` defaults at the DB level, no draft
  state), and the single pre-seeded `kb.publish` permission gates the whole
  CRUD surface including reads — a real, documented scope limit, not a
  broader company-wide read grant. See all seven entries below for full
  detail. **Domain H (Supporting Operations, #66–74) is now COMPLETE** —
...
- **Part D — PDPL / M-series — COMPLETE, all nine backlog items built.** **M03 Consent
  ...
  `IncidentReport`, a Material classification's DPO-classify + Executive-Management-cosign
  maker/checker pair (the first permission in this codebase shared by both sides of a
  pair, segregated by an in-service role check plus `assertDifferentActors` plus a DB
...
**Part C #53-54 — Operational & Professional Indemnity Risk (Domain F, Process 53-54)** —
  two checkboxes: "A generic risk register covering the six categories the source
  explicitly names: operational/cyber/financial/compliance/reputational
  (`RiskRegisterItem.riskType`) — Professional Indemnity gets its own, deeper table" and
  "Track the broker's own Professional Indemnity policy (coverage limit, expiry, claims
  history) — a broker without valid PI cover is itself a licensing breach." Three
  independent-but-related resources under Part 5/7.1: the generic risk register
  (`RiskRegisterItem`), the broker's own PI policy (`ProfessionalIndemnityPolicy`), and PI
  risk events (`ProfessionalIndemnityRiskEvent`). All three pre-existed as bare "core
  schema" models — **genuinely no migration, no seed change** —
  `risk-register.manage` (`[COMPLIANCE_OFFICER, BRANCH_DEPARTMENT_MANAGER]`) /
  `pi-policy.manage` (`[COMPLIANCE_OFFICER]`) were pre-seeded ahead of time.

  **`RiskRegisterItem`** covers exactly the five non-PI categories — `riskType` is
  `@IsIn`-validated against `operational | cyber | financial | compliance | reputational`
  only, with no `professional_indemnity` value accepted at all. Not a
  `WorkflowTransitionService` entity, no maker/checker — a factual log (`status`: plain
  string `open -> closed`, the `RetentionCase`/#46 shape). `POST /risk-register`
  (`loggedAt` backdatable via `parseHistoricalInstant`, the #10/#12/#44/#45 shape),
  `POST /risk-register/:id/mitigation` (legal only while `open`, status-conditional),
  `POST /risk-register/:id/close` (idempotent), `GET /risk-register?riskType=&status=` +
  `/:id`.

  **`ProfessionalIndemnityPolicy` is deliberately NOT a fixed-id singleton like
  `BrokerLicense`** (#51) — the model has no `issuedAt`/period-start field to define a
  validity window with, so a renewal is a brand-new row (preserving `claimsHistorySummary`
  per period as its own historical fact, rather than a renewal silently overwriting it) and
  "current" is simply the row that expires furthest in the future
  (`findFirst({ orderBy: [{ expiresAt: 'desc' }, { id: 'desc' }] })`). This definition
  pre-dates this process: `PolicyCheckingRepository.findLatestPiPolicyId` (Process 20/54's
  discrepancy auto-link) already needed it before `PiPolicyRepository` existed to give it a
  home — that method now delegates to `PiPolicyRepository.findCurrent()` instead of
  carrying its own copy of the query, so there is exactly one definition of "current" for
  both callers. `POST /pi-policy` (always inserts a new row), `GET /pi-policy/current`
  (404 until one exists — a tracking record, not an issuance gate, so nothing else in this
  codebase depends on a configured value existing), `GET /pi-policy` (list, each row
  flagged `isCurrent`/`isCurrentlyLapsed`), `GET /pi-policy/:id`,
  `POST /pi-policy/:id/claims-history` (overwrites `claimsHistorySummary` on ANY row, not
  just the current one — the `UPDATE` audit row is the history of how the summary changed
  over time, not a bespoke revision field).
  ...
  **`ProfessionalIndemnityRiskEvent` already had a real writer before this landing** —
  ...
  This process gives those rows their first read/list surface
  (`GET /pi-risk-events?piPolicyId=&sourcePolicyCheckingId=` + `/:id`) plus a manual-log
  path for an exposure that did NOT come through a Policy Checking discrepancy
  (`POST /pi-risk-events` — `piPolicyId` optional, auto-resolving to the current PI policy
  when omitted, exactly the auto-link's own default) and a
  `POST /pi-risk-events/:id/mitigation` endpoint.

  **`@code-reviewer` (mandatory — a shared repository refactor touching a maker/checker-
  adjacent module + a new derived-"current" resolution over money/PII-adjacent data) →
  CHANGES REQUESTED → resolved: 1 BLOCKER + 1 MAJOR.**

  - **BLOCKER**: `PiPolicyRepository.findCurrent()`'s original
    `orderBy: { expiresAt: 'desc' }` had no secondary sort key, and a tie is the LIKELY
    case, not a theoretical one — `CreatePiPolicyDto.expiresAt` goes through
    `parseCalendarDate`, so any plain-date input (the expected shape) normalizes to an
    exact UTC midnight; two officers entering the same calendar-year renewal date, a
    genuine multi-insurer PI tower with layers expiring the same day, or an accidental
    duplicate submission (no uniqueness guard exists on this table) all tie to the
    millisecond, and Postgres gives no guarantee which tied row an unordered
    `ORDER BY ... LIMIT 1` returns — so `GET /pi-policy/current`, `list()`'s `isCurrent`
    flag, and the Policy Checking discrepancy auto-link could each independently, and
    inconsistently across calls, pick a DIFFERENT "current" record. Fixed by adding
    `id: 'desc'` as a deterministic secondary sort key on both `findCurrent()` and
    `findMany()`.
  - **MAJOR**: the new `piRiskEventAuditSnapshot` embedded an auto-logged event's
    `description` verbatim into the `AuditLogEntry` — but for an auto-logged event
    (`sourcePolicyCheckingId` set) that text is `piRiskEventDescription()`'s exact
    coverage-figure diff (requested vs. issued amounts, plus the policy number) — the SAME
    content `policyCheckingAuditSnapshot` deliberately keeps OUT of the sibling
    `PolicyChecking` audit row (`ibms-brain/meta/lex/sensitive-data-handling.md` —
    "metadata not body," the #12-19 precedent this module was about to violate one field
    over). Fixed by redacting `description` to a placeholder whenever
    `sourcePolicyCheckingId` is set, leaving a MANUAL entry's own narrative verbatim
    (`mitigationAction` is never redacted either way — it is always fresh text a human
    wrote afterward, not derived content).
  - **2 MINORs**: a manual risk event auto-resolving to a currently-lapsed PI policy
    (`piPolicyId` omitted, `findCurrent()` returns a lapsed record) previously had no
    signal anywhere that the linked cover was not current — rather than adding an
    `isCurrentlyLapsed` field to every future read of `PiRiskEventView` (which would need
    either an N+1 lookup per event or a bulk-fetch on every `list()` call, for a fact only
    ever true at the moment of creation), the signal is now durable in two lighter-weight
    ...
    survives even after the PI policy is eventually renewed. Separately, the
    claims-history update's lack of an optimistic-concurrency guard is now an explicit
    stated trust-level decision in `pi-policy.service.ts`'s own comment (the exact
    `BrokerLicense.scopeOfAuthorization` reasoning), not left implicit.
  - **1 deferred MINOR (not fixed)**: none of the three models carry a
    `DataClassification` field. Unlike #49's BLOCKER 4 fix for `WatchlistEntry` (a brand
    new model in that PR), this predates the PR — no migration lands here at all, and
    properly resolving it would mean widening three pre-existing models rather than the
    narrow, checkbox-scoped work this process asks for. Flagged as a real open question,
    not resolved by extending the `BrokerLicense.scopeOfAuthorization` analogy any
    further: `ProfessionalIndemnityPolicy.claimsHistorySummary` and an auto-logged risk
    event's underlying coverage-figure content plausibly name identifiable customers and
    settlement amounts, closer in profile to `TransactionMonitoringAlert.detailText`
    (`HIGHLY_CONFIDENTIAL`) than to a bare authorization-scope label — a real PCMS/
    `PRIV-STD-02` determination is needed here, not another engineering guess.

  Verifying the tiebreak fix also caught a genuine cross-run test-monotonicity bug: the
  new e2e spec's own `nextFarFuture()` helper (needed because "current" resolution is
  book-wide with no per-test scoping possible, against a db-test database that is
  cumulative across runs) originally grew its offset by a whole extra DAY per call within
  a run — which broke monotonicity ACROSS runs, since a later-position call in an earlier
  run could still out-rank an earlier-position call in a later run whenever the wall-clock
  gap between the two runs was under a day (always true in practice). Fixed by keeping the
  base offset FIXED and using only `Date.now()` (strictly increasing between any two runs)
  plus a same-run millisecond counter as the tiebreak.

  `apps/web/` gains a new **"Operational & PI risk"** screen
  (`app/(app)/operational-pi-risk/page.tsx` + `lib/compliance-risk/risk-register-api.ts` +
  `pi-policy-api.ts` + `pi-risk-event-api.ts` + an `AppNav` entry after "Regulatory
  compliance") — one page, three sections.

  **Verification**: +47 api unit (`risk-register.config.spec.ts` 5,
  `risk-register.service.spec.ts` 11, `pi-policy.config.spec.ts` 7,
  `pi-policy.service.spec.ts` 8, `pi-risk-event.config.spec.ts` 4,
  `pi-risk-event.service.spec.ts` 12) → api unit **1752** (123 files, from 1705). New
  `test/operational-pi-risk.e2e-spec.ts` **4/4** — risk register CRUD + the PI-category
  exclusion (400 on `riskType: 'professional_indemnity'`) + mitigation-while-open +
  idempotent close + list filters; PI policy create/claims-history/current/list; a
  deliberate tie-on-`expiresAt` regression test (two records sharing the exact same
  `expiresAt`, then 5 repeated `/current` reads all agreeing on the same winner, matching
  `list()`'s `isCurrent` flag); manual PI risk event log (explicit `piPolicyId` + the
  auto-resolve default) + mitigation + list filters. `test/policy.e2e-spec.ts`'s
  pre-existing discrepancy-to-PI-risk-event auto-link (Process 20) re-verified unaffected
  by the `PolicyCheckingRepository` refactor — confirmed both by direct code inspection
  (the refactor is behaviorally identical to the pre-existing inline query) and by running
  that file in isolation. Full api e2e suite green aside from the 2 documented chronic
  flakes — `rbac.e2e-spec.ts` confirmed passing cleanly in isolation; `up-sell.e2e-spec.ts`
  confirmed as a Prisma engine-connection/timeout error (`Response from the Engine was
  empty` / `Engine is not yet connected`), not an assertion failure, and entirely unrelated
  to this diff (up-sell shares no code with compliance-risk/policy-checking). New
  Playwright `operational-pi-risk.spec.ts` 3/3; full Playwright suite **171/171** (from
  168). `npm run typecheck`/`lint`/`build` OK on both api and web.

  **Deferred**: the backlog checkbox says "track," not "block" — there is deliberately no
  automated new-business-blocking gate for lapsed PI cover, unlike #51's explicit
  `BrokerLicense` gate; building one would be a separate, deliberate product decision. No
  `DataClassification` field on any of the three models (see above — a real open gap, not
  resolved in this pass). No renewal-history UI beyond the plain list + `isCurrent` flag.
  `risk-register.manage`/`pi-policy.manage` are role-level (no per-officer queue). No
  retention/disposal policy for PI risk data.

**Part C #55 — Incident Management (Domain F, Process 55)** — three checkboxes: the full
  state machine "Reported→Contained (4-hour target for critical)→Impact Assessed→Classified
  (Material requires DPO + Senior Management co-sign)→Notified→Recovered→Closed (root cause
  mandatory)," "Senior management notification within 1 hour of Material classification
  (job)," and "Independent multi-regulator notification (CBJ, National Cybersecurity Centre,
  Personal Data Protection Council) since one incident may trigger more than one regulator's
  obligations." **This build is also Part D's M09 (Incident & Breach Management)** —
  `ibms-brain/meta/context/pcms-privacy-modules.md`'s own one-line summary and
  governing-document citation for M09 (`Governing Policy §12`, `PRIV-SOP-09`) match
  `IncidentReport`'s own Prisma doc comment verbatim, so this one build satisfies both
  backlog references, the same "one build, two backlog entries" shape #16/#50 established
  for Conflict of Interest (inverted here: there #50 needed no separate build; here ONE
  build covers both #55 and M09).

  **`IncidentReport` (core schema) pre-existed with every field this needs**, including
  both maker/checker columns (`classifiedByDpoUserId`, `seniorManagementCoSignUserId`) —
  genuinely almost no migration. `WORKFLOW_TRANSITIONS.IncidentReport` (a strictly linear
  7-state chain, no branching) and `SLA_REGISTRY`'s `incident_containment` (4 hours,
  critical severity only) / `incident_senior_management_notification` (1 hour, Material
  only) also pre-existed since the A.8 SLA registry seed — this module is their first real
  consumer. The one migration this process adds, `20260906120000` (47th), bolts on ONLY the
  `IncidentReport_classification_maker_checker_distinct` CHECK constraint. **No seed
  change** — four permissions were pre-seeded ahead of time: `incident.report` (broad — 8
  roles: Sales/Placement/Claims/Finance/Compliance/Manager/Admin/DPO), `incident.contain`
  (Admin/Compliance), `incident.classify` (DPO **and** Executive Management), and
  `incident.notify-regulator` (DPO/Compliance).

  **`incident.classify` is the first permission in this codebase's entire seed grid shared
  by BOTH roles of a maker/checker pair**, rather than two distinct codes the way every
  other pair here works (`dsr.handle`/`dsr.close`, `kyc.capture`/`kyc.approve`, ...). The
  segregation is enforced one layer deeper: `IncidentService.classify()` explicitly checks
  ...
  the new DB CHECK. A dedicated unit test proves this holds even for a single user who
  happens to hold BOTH roles simultaneously (nothing in RBAC structurally prevents a dual
  assignment): `classify()` passes that user's DPO check and stamps
  `classifiedByDpoUserId = actor.id`; `coSign()` then passes their EXEC check, but
  `assertDifferentActors` rejects it because the two ids match.
  ...
  **The "senior management notification (job)" checkbox has NO bespoke scheduler.**
  ...
  MANUAL stamp — confirmed via a repo-wide grep that zero real email/SMS/notification-
  sending infrastructure (`sendEmail`/`EmailService`/`NotificationService`/`sendSms`/
  `nodemailer`) exists anywhere in `apps/api/src`, the same `CommunicationLog`
  ...
  `SlaTimerScheduler` (every 15 minutes, backlog A.8 — `IncidentReport` was already listed
  in `SLA_DASHBOARD_SENSITIVE_ENTITY_TYPES` before this process touched anything) is the
  "(job)" that catches it, escalating the unresolved timer past the 1-hour deadline onto
  the #43 SLA dashboard. Building a second, bespoke scheduler here would duplicate
  infrastructure that already exists and already does exactly what this registry entry
  needs (no named escalation target beyond a bare "—", unlike DSR's two-stage
  DPO-then-General-Manager escalation).

  **The Material co-sign gate is re-derived LIVE, in two places** — the #16/#51 pattern.
  Both `notifyRegulators()` (`CLASSIFIED → NOTIFIED`, one of #55's three named checkboxes)
  and `notifyAffectedSubjects()` (M09's own PDPL scope, not one of the three, but a
  plausible field the model itself already carries) independently re-check
  `classification === 'MATERIAL' && !seniorManagementCoSignUserId` immediately before their
  own write, rather than trusting a value cached from whenever `classify()` ran. This is
  safe against a genuinely concurrent double-call without needing to re-assert the co-sign
  inside the engine's own status-conditional `updateMany` `WHERE`, because classification
  and the co-sign are both write-once/monotonic fields on this entity — nothing can un-set
  a recorded co-sign between the guard check and the transition.

  **`@code-reviewer` (mandatory — a new maker/checker pair + a new DB CHECK migration + a
  new `WorkflowTransitionService` entity + Highly-Confidential-adjacent breach data) →
  CHANGES REQUESTED → resolved: 1 MAJOR + several MINORs + a NIT.**

  - **MAJOR**: `title` was missing the same `NO_FULL_ACCOUNT_NUMBER` guard `description`
    already had. A reporter's first, least-considered input during an active incident is
    exactly the field most likely to get sensitive detail typed into it before a fuller,
    guarded description is written, and it is embedded verbatim into every CREATE/UPDATE
    audit row plus every `incident.report`-holder's read (eight roles) and the web table.
    Fixed by adding the same `@Matches` guard to `title`.
  - **MINORs**: `ibms-brain/meta/lex/maker-checker-segregation.md` gained an
    `IncidentReport` row plus a new "shared permission across two roles" exception clause,
    documenting this as an accepted, deliberate exception rather than a violation, and
    recommending new pairs default to two distinct permission codes unless there's a
    specific reason to share one; `ibms-brain/meta/context/incident-management.md` didn't
    exist at review time — the #51 MAJOR-3 lesson again ("documentation is part of done"),
    fixed by writing it; `notifyAffectedSubjects()` was missing the co-sign gate
    `notifyRegulators()` had — an asymmetry between two equally consequential, irreversible
    external actions for a Material incident, with no principled reason, fixed for parity;
    the web page showed the Classify buttons to an Executive-Management-only user and the
    Co-sign button to a DPO-only user, both an always-403 dead end server-side — fixed by
    splitting button visibility (`isDpo`/`isExec`) to the specific role each sub-action
    actually needs.
  - **NIT**: added the explicit dual-role unit test described above (previously only
    inferable from the same-actor test, not directly proven).

  Also caught and fixed twice during this build, unrelated to the review: a literal
  `*/15 * * * *` cron string written inside a `/** ... */` JSDoc comment prematurely closes
  the comment block at that `*/`, corrupting all subsequent parsing until EOF (manifesting
  as bizarre cascading syntax errors from `npm run typecheck`) — reworded to "runs every 15
  minutes" in both `incident.service.ts` and `compliance-risk.module.ts`.

  `apps/web/` gains a new **"Incident Management"** screen
  (`app/(app)/incidents/page.tsx` + `lib/compliance-risk/incident-api.ts` + an `AppNav`
  entry) — a create form plus a per-row action pipeline through all 7 states, with
  role-gated co-sign/notify-senior-management/notify-regulators/notify-affected-subjects
  sub-actions.

  **Verification**: +47 api unit (`incident.config.spec.ts` 10, `incident.service.spec.ts`
  37 incl. the review-fix regression tests) → api unit **1799** (125 files, from 1752). New
  `test/incident.e2e-spec.ts` **3/3** — a full CRITICAL incident walked through MATERIAL
  classification, co-sign, senior-management notification, and multi-regulator
  notification; a NON_MATERIAL classification skipping the co-sign gate entirely (and
  blocking co-sign outright); permission/list-filter coverage — extended post-review to
  assert BOTH `notify-regulators` and `notify-affected-subjects` return 422 before co-sign
  is recorded. New Playwright `incidents.spec.ts` **4/4** — extended post-review: a DPO
  sees "Notify Senior Management" but not "Co-sign (Senior Management)"; a new test
  confirms only Executive Management sees the Co-sign control. `npm run typecheck`/`lint`/
  `build` (api + web) OK.

  **Deferred**: senior-management notification and regulator/affected-subject notification
  are all manual stamps, not real sends — no email/SMS/paging integration exists anywhere
  in this codebase, and building one is a distinct, separate infrastructure decision, not
  part of #55. No `DataClassification` field on `IncidentReport` itself (the model
  pre-dates this process — unlike #49's `WatchlistEntry` BLOCKER, this isn't a new model
  this PR introduces without one). `incident.classify`'s shared-permission maker/checker
  shape is a deliberate, documented exception, not the new house style — new pairs should
  still default to two distinct permission codes. All four `incident.*` permissions are
  role-level (no per-officer queue). No retention/disposal policy for incident data.

**Part C #56 — Internal Controls / Maker-Checker (Domain F, Process 56)** — a one-liner
  backlog item, not a checkbox list: "fully covered in Part A.5, plus a periodic audit
  report scanning for any possible self-approval cases." Part A.5 built the actual
  controls back near the start of this codebase's history — a generic
  `assertDifferentActors(makerId, checkerId, context)` helper (`common/maker-checker.
  util.ts`) called at every checker write path, plus a DB `CHECK` constraint per pair
  as the structural backstop. This process re-verified that coverage is genuinely
  intact, then built the ONE thing the backlog line adds that didn't already exist: the
  report.

  **`MAKER_CHECKER_REGISTRY`** (`internal-controls.config.ts`) is a hand-built inventory
  — one entry per DB `CHECK` constraint this codebase has added, traced back across
  five separate migrations (`20260826091424_add_maker_checker_check_constraints`,
  `20260827120000_add_needs_assessment_status_enum`,
  `20260903170000_add_complaint_management`, `20260905120000_add_dsr_widening`, and
  `20260906120000_add_incident_maker_checker_check`): `KYCRecord`
  (`createdByUserId`/`approvedByUserId`), `PolicyChecking`
  (`placedByUserId`/`checkedByUserId`), `Refund`
  (`raisedByUserId`/`approvedByUserId`), `DisposalBatch`
  (`nominatedByUserId`/`dpoApprovedByUserId`), `DataSharingApproval`
  (`requestedByUserId`/`approvedByUserId`), `DataProcessingAgreement`
  (`assessedByUserId`/`dpoApprovedByUserId`), `Settlement`
  (`approvedByUserId`/`secondApproverUserId`), `CommissionLedgerEntry`
  (`overrideRequestedByUserId`/`overrideApprovedByUserId`), `Recommendation`
  (`draftedByUserId`/`approvedByUserId`), `AccessRecertificationItem`
  (`subjectUserId`/`reviewerUserId`), `NeedsAssessment` (`createdByUserId` against BOTH
  `reviewedByUserId` and `approvedByUserId` — two separate constraints, two registry
  entries), `Complaint` (`resolvedByUserId`/`closureApprovedByUserId`),
  `DataSubjectRequest` (`processedByUserId`/`closedByUserId`), and `IncidentReport`
  (`classifiedByDpoUserId`/`seniorManagementCoSignUserId`) — 15 entries in total.
  **`DisposalBatch`/`DataSharingApproval`/`DataProcessingAgreement` are flagged
  `dormant: true`** — no application code writes to any of the three yet (M06/M07/M08
  aren't built; see this document's own Part D gaps above) — kept in the registry and
  scanned anyway rather than removed, the #48 `third_party_payment_source`
  dormant-classifier precedent: the day one of those three modules lands, this report
  already covers it with no further change needed.

  **A 16th pair is handled separately, outside the registry, because it needs a
  different query shape**: `PolicyChecking.checkedByUserId` vs the PARENT
  `Policy.issuedByUserId`. This resolves an open question
  `ibms-brain/meta/context/policy-lifecycle.md` had flagged since Process 20 shipped:
  `PolicyCheckingService.check()` enforces `checkedByUserId != issuedByUserId`
  app-side (`assertDifferentActors`) as a "stricter-than-lex belt," but the
  `PolicyChecking_maker_checker_distinct` `CHECK` only ever covered
  `placedByUserId` — should the `CHECK` extend to `issuedByUserId` too? **The answer,
  settled here, is that it structurally cannot**: a Postgres `CHECK` constraint can
  only compare columns on the SAME row of the SAME table, and `checkedByUserId`
  (`PolicyChecking`) and `issuedByUserId` (the parent `Policy`) live on different
  tables. Closing this gap with a real DB-level mechanism — a trigger (this codebase
  already has one precedent, the `AuditLogEntry` immutability trigger) or a
  denormalized copy of `issuedByUserId` kept in sync on `PolicyChecking` — is a real,
  deliberate follow-up decision, explicitly NOT made here; in the meantime, **this
  periodic scan is that one pair's ONLY defense-in-depth backstop** beyond the
  app-level guard, by design, not an oversight.

  **The scan itself**: each of the 15 registry pairs `findMany`s its table with **NO
  `where` filter** — every row is fetched (capped at
  `INTERNAL_CONTROLS_SCAN_LIMIT = 20000`, a `truncated` flag set if hit) — and
  null-checking happens entirely inside the pure `classifyPairRows` function instead of
  the query. This was a deliberate choice, not an oversight: `AccessRecertificationItem.
  reviewerUserId` is the one checker field that is NOT NULL, while every other checker
  field IS nullable, and trusting 15 different Prisma-generated filter types to accept
  `{ not: null }` identically — behind an `unknown` cast that already bypasses
  TypeScript's own per-model field checking, since the registry drives a single generic
  code path across 15 different models by name — was judged a real runtime risk not
  worth taking for tables this size (workflow/approval records, not transactional
  ledgers). The cross-table pair follows the `policy` relation instead of a second
  column on the same row, through the identical classification logic.

  **A genuine performance fix was found mid-build, not in a review pass**: the 16
  pair-scans originally ran sequentially (a `for...of` loop, one `await` at a time),
  which timed out a 30-second e2e test against this session's own long-lived,
  cumulative `db-test` database (the "never assert a global row count — it's
  cumulative across the whole suite's history" lesson, this time surfacing as a
  latency problem rather than a false assertion). Refactored to fire all 16 scans
  **concurrently** via `Promise.all`, cutting the same scan from timing out past 30
  seconds to under 12. This is a real production-quality fix, not merely a test
  accommodation — a self-approval report that takes tens of seconds to compute is bad
  regardless of what a test's timeout happens to be set to.

  A best-effort `READ` `AuditLogEntry` row is always written on a run (counts only —
  `pairsScanned`/`totalRowsChecked`/`violationCount`, never a user id). **If any
  violation is found — the not-expected outcome, see below — it is logged at `ERROR`
  and persisted as one `CREATE` `InternalControlsViolation` row per violation**
  (entity/field/user ids), reusing `AuditLogEntry`'s own polymorphism rather than a new
  model, the #48 `TransactionMonitoringAlert` record-keeping choice repeated.
  `GET /internal-controls/self-approval-audit` (new permission
  `internal-controls.view` — `[COMPLIANCE_OFFICER, EXECUTIVE_MANAGEMENT,
  EXTERNAL_AUDITOR]`, genuinely new rather than pre-seeded ahead of time the way #55's
  four were, 148 → 149 total permissions; seeded as `internal-controls.audit`
  originally, renamed 2026-09-06 — every EXTERNAL_AUDITOR grant must end in
  `.read`/`.view`, and this route was always a plain read) runs the scan on demand;
  `InternalControlsAuditScheduler` runs the identical scan nightly at 10:00 UTC (after
  the 09:00 transaction-monitoring sweep), resolving the `system@ibms.internal` account
  the same way every other nightly sweep in this codebase does.

  **Why a clean run is the EXPECTED outcome, not a surprise, and what that meant for
  testing**: every one of the 15 same-table pairs is already backed by a DB `CHECK` —
  a self-approval on any of them should be structurally impossible regardless of which
  code path writes the row, app service, a future integration, or raw SQL. This report
  exists as independent verification, not detection of a condition expected to occur —
  an auditor should be able to see "we scanned, and found none," not merely be told
  "the code has a guard." That shaped the whole test strategy: proving GENUINE
  end-to-end detection meant deliberately targeting the one pair with no DB backstop
  (the cross-table `PolicyChecking`/`Policy` pair) — a violation was planted directly
  via Prisma, bypassing `assertDifferentActors` entirely (which Postgres itself would
  reject on any of the other 15 pairs, DB CHECK or no service code involved at all),
  confirmed detected by the live scan through the real HTTP endpoint, then restored to
  a clean state before the test finished — the #51 `BrokerLicense`-singleton
  restore-before-finishing precedent, scaled down to one row. Every same-table pair's
  own classification logic is instead proven correct via pure-function unit tests (a
  mocked row with `maker === checker` is flagged; Postgres would reject that same row
  for real, so an e2e-level proof for those 15 would need to bypass the database
  itself, which isn't meaningful).

  `apps/web/` gains a new **"Internal controls"** screen
  (`app/(app)/internal-controls/page.tsx` + `lib/internal-controls/internal-controls-
  api.ts` + an `AppNav` entry) — stat cards (pairs scanned / rows checked /
  violations), a violations table shown only when the count is non-zero, a by-pair
  breakdown table (entity, pair, rows checked, violation count, the backing DB `CHECK`
  name or `—`, a `(dormant)` marker), and a manual "Run audit now" button.

  **A `react-hooks/set-state-in-effect` ESLint false positive was hit and worked
  around, worth recording since the root cause was never fully explained by the rule's
  own diagnostics** — only empirically bisected against a known-working file. `void
  load();` called directly inside a mount `useEffect` was flagged as "calling setState
  synchronously within an effect," even though every `setState` call inside `load`
  happens strictly after an `await` — the identical shape several OTHER pages in this
  codebase (`sla-dashboard`, for one) already use without triggering the rule. Removing
  pieces one at a time never made the flag go away; the fix that actually worked,
  already present as prior art in `apps/web/app/(app)/incidents/page.tsx`, was wrapping
  the call as `void (async () => { await load(); })();` instead of `void load();` — an
  extra layer of indirection the rule's static analysis apparently does trace through
  correctly where a bare identifier call does not.

  **Verification**: +21 api unit (`internal-controls.config.spec.ts` 15 —
  registry-integrity checks, `classifyPairRows`/`classifyCrossTableRows` boundary
  cases, `buildInternalControlsReport` aggregation; `internal-controls.service.spec.ts`
  6 — a full clean scan across all 16 pairs, a detected same-table violation, the
  cross-table violation, dormant-pair reporting, a failed audit write not aborting the
  scan, the scheduled-audit delegation) → api unit **1820** (127 files, from 1799). New
  `test/internal-controls.e2e-spec.ts` **2/2** — permission gating (403 for an
  outsider) plus the full 16-pair report shape (including the `dormant`/no-DB-CHECK
  markers); the live-planted cross-table violation walked end-to-end through the real
  HTTP endpoint, its `InternalControlsViolation` audit row confirmed, then restored.
  Full api unit suite 1820/1820 confirmed green (127 files, no regressions). New
  Playwright `internal-controls.spec.ts` 4/4 (a clean report render, a violation
  rendered prominently, the 403 friendly message, `@a11y`). `npm run
  typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: no real-time alerting on a finding beyond the audit log + the next
  scheduled/on-demand read — a violation does not push a notification anywhere.
  Closing the `PolicyChecking`/`Policy` cross-table gap with a real DB-level mechanism
  (a trigger, or a denormalized column) is a deliberate, undone follow-up — this scan
  is the compensating control, not a substitute for eventually building one. A new
  maker/checker pair added to the schema after this process ships needs a
  `MAKER_CHECKER_REGISTRY` entry added by hand — nothing discovers pairs automatically
  from the schema. `internal-controls.view` is role-level (no per-officer queue). A
  violation found here does not create an `InternalAuditFinding` row — #57 (Internal
  Audit) is a separate, not-yet-built process this report does not feed.

**Part C #57 — Internal Audit (Domain F, Process 57)** — **closes Domain F: #47–57 are
  all built or verified-covered.** Two checkboxes: "Record audit findings, remediation
  path, and closure" and "Time-boxed read-only access for the External Auditor role
  across all records, documents, and workflow history." Built as two separate modules,
  since they are genuinely separate deliverables sharing one backlog number.

  **Checkbox 1 — `InternalAuditFinding`.** The model (core schema) is the exact same
  bare "generic register" shape as `RiskRegisterItem` (#53-54), one model up in the same
  `schema.prisma` file: `auditPeriodLabel`/`finding`/`remediationAction`/`status`/
  `loggedAt`/`closedAt`, no maker/checker columns at all. Not a `WorkflowTransitionService`
  entity — `status` is a plain string `open -> closed`. **`internal-audit.record`
  (`[COMPLIANCE_OFFICER]`) and `internal-audit.close` (`[COMPLIANCE_OFFICER,
  BRANCH_DEPARTMENT_MANAGER]`) are two DISTINCT permissions, not a maker/checker pair** —
  the model carries no maker/checker columns for `assertDifferentActors` to enforce
  anything against. This is a deliberate authority split (only Compliance may RECORD a
  finding or update its remediation plan; a Manager may also see one through to
  CLOSURE), the same "two permissions, two different actions, no dual-approval
  relationship between them" shape as #48's `aml.monitor`/`aml.escalate`. Reads (`GET`)
  accept EITHER permission — a Manager reviewing before closing needs to see the finding
  too. Genuinely no migration, no seed change — both permissions were pre-seeded ahead
  of time.

  **Checkbox 2 — the External Auditor's read-only lens.** `ibms-brain/meta/context/
  roles-and-segregation-of-duties.md`'s own Part 5.1 role table names the scope
  precisely: "Read-only access to logs, documents and workflow history for a defined
  engagement period" — not blanket read access to every business table's live content.
  The backlog's own phrasing ("across all records, documents, and workflow history")
  reads more broadly at first glance, but the pre-seeded permission grid had already
  settled this the narrower way, before this process ever ran: `audit-log.read`,
  `document-history.read`, and `workflow-history.read` were all seeded ahead of time
  (module `compliance-risk`), and none of them is a generic `customer.read`/
  `policy.read`/`claim.read`-style grant. This process is simply their first real
  consumer. **"All records" is satisfied by `AuditLogEntry` itself being polymorphic
  across every entity type in the schema** — an auditor filters the SAME table down to
  whichever record they're reviewing, rather than being handed live read access to each
  business table directly — a materially smaller, more defensible grant than the
  alternative reading, and one that matches Part 5.1's own "Cannot: Modify any record"
  framing: an auditor with `audit-log.read` still cannot open a `Customer`/`Policy`/
  `Claim` record directly through any endpoint that permission gates; they can only read
  the audit trail ABOUT it. **"Time-boxed" was already fully built and tested before
  this process existed** — `User.accessValidUntil` + `SessionService.validate` revoke an
  active session, log an `ACCESS_WINDOW_EXPIRED` audit row, and throw the moment the
  window passes (the Auth e2e's own "External Auditor time-boxed access (Part 5.1)"
  suite) — a generic, per-user mechanism, not something specific to `EXTERNAL_AUDITOR`
  that needed building here.

  **New module** `apps/api/src/modules/audit-trail/` (`AuditTrailController` +
  `AuditTrailService` + `AuditTrailRepository`) — a read-only, cross-cutting lens, the
  `SlaDashboardModule` shape (aggregates other modules' data, owns none of it). Three
  routes, all writing a best-effort `READ` audit row unconditionally
  `isSensitiveDataAccess: true` (no per-entityType classification the way the SLA
  dashboard's `hasSensitiveEntityType` does — the audit/document/workflow history of an
  arbitrary record can surface Highly Confidential content regardless of which specific
  entity is being inspected):

  - `GET /audit-trail?entityType=&entityId=&userId=&action=&from=&to=` (`audit-log.read`
    — `[COMPLIANCE_OFFICER, SYSTEM_SECURITY_ADMINISTRATOR, EXTERNAL_AUDITOR]`) — the
    general browse, every filter optional, newest-first, capped at
    `AUDIT_TRAIL_READ_LIMIT = 5000`.
  - `GET /audit-trail/workflow-history?entityType=&entityId=` (`workflow-history.read` —
    `[COMPLIANCE_OFFICER, EXTERNAL_AUDITOR]`, no Admin) — `AuditLogEntry` rows for that
    one record where `action = 'TRANSITION'`, chronological — literally
    `WorkflowTransitionService`'s own audit trail, read back for the first time by
    anything other than the engine itself.
  - `GET /audit-trail/documents/:id/history` (`document-history.read` —
    `[COMPLIANCE_OFFICER, EXTERNAL_AUDITOR]`, no Admin) — walks the `Document` version
    chain (`previousVersionId`/`nextVersion`, a doubly-linked list,
    `previousVersionId @unique`) in BOTH directions from the requested id, plus every
    `AuditLogEntry` row for `entityType: 'Document'` across every id in that chain.

  **`SYSTEM_SECURITY_ADMINISTRATOR` holds `audit-log.read` but NEITHER
  `document-history.read` NOR `workflow-history.read`** — a real, seed-level distinction
  worth remembering: Admin can browse the general log (their own access is itself
  supposed to be logged and periodically reviewed, Part 5.1) but the two narrower
  historical lenses are Compliance/Auditor-only.

  **The `Document` version chain is dormant today** — no application code creates a
  second version of a document yet (`versionNumber` stays `1`, `previousVersionId`/
  `nextVersion` stay `null` for every row this codebase has written so far). Most
  documents are their own whole one-row chain right now. This is a forward-compatible
  walk, not dead code — the #48/#56 dormant-feature precedent: the day something starts
  versioning documents, this endpoint already covers it with no further change.
  `DOCUMENT_VERSION_CHAIN_WALK_LIMIT = 1000` is a pure safety valve against a data
  anomaly (a cycle) turning the walk into an infinite loop, not a realistic
  chain-length expectation.

  `apps/web/` gains two new screens: a new **"Internal audit findings"** screen
  (`app/(app)/internal-audit-findings/page.tsx` — a log form, a table with per-row
  remediation/close actions gated by role) and a new **"Audit trail"** screen
  (`app/(app)/audit-trail/page.tsx` — three lookup sections: a filterable audit-log
  browse, a workflow-history lookup by entity type/id, and a document-history lookup by
  document id, each independently showing a friendly 403 message).

  **Verification**: +26 api unit (`internal-audit-finding.config.spec.ts` 4 —
  view/audit-snapshot mapping; `internal-audit-finding.service.spec.ts` 11 — create/
  remediation/close incl. 404/409/idempotency; `audit-trail.config.spec.ts` 5 — the
  version-chain ordering + requested-version flag; `audit-trail.service.spec.ts` 6 —
  all three read methods incl. the 404 on an unknown document and a failed audit write
  not aborting the read) → api unit **1846** (131 files, from 1820). New
  `test/internal-audit.e2e-spec.ts` **2/2** — the finding CRUD's per-action permission
  split proven end-to-end (Compliance records/remediates, a Manager can close but not
  record/remediate, an outsider can do neither, a byte-identical re-close is
  idempotent); the External Auditor's three reads proven against a REAL `Lead`
  TRANSITION (`POST /leads` + `POST /leads/:id/transition` — the simplest real
  workflow-transition fixture in the whole codebase, deliberately chosen over Policy/
  Claim's much heavier setup) and a REAL `Document` + matching `AuditLogEntry` seeded
  directly via Prisma, including confirming Admin gets 403 on document/workflow history
  but 200 on the general browse, and a 404 on an unknown document id. New Playwright
  `internal-audit-findings.spec.ts` 4/4 + `audit-trail.spec.ts` 3/3. `npm run
  typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: no export/print capability over the audit trail — Part 10.6's
  watermarking/export-restriction rules (`ibms-brain/meta/lex/sensitive-data-
  handling.md`) would need to be satisfied first if one is ever added; today these are
  read-only JSON API responses only. No UI/tooling to actually SET `accessValidUntil` on
  an External Auditor account for a defined engagement — the field and its enforcement
  exist; provisioning tooling around it is a separate, undone concern. Extending
  `workflow-history.read`/`document-history.read` to Admin, or `audit-log.read` to any
  other role, is a real, deliberate seed decision left alone, not an oversight to fix.
...
**Part C #61 — Employee Performance (Domain G, Process 61)** — the fourth Domain G
  item: "`EmployeePerformanceRecord`: a periodic job: new clients/premium/commission/
  renewal rate/cross-sell rate." Like #60, `EmployeePerformanceRecord` already existed
  in the core schema — this process is its first real consumer, alongside TWO other
  dormant pieces: `Employee` itself (Domain H/#66 HR is not built — no application
  code has ever created one) and `User.employeeId` (the FK that would link a login
  account to one — no writer sets it either).

  **Resolving "employee" to something scoreable.** Every business signal this process
  needs (`Lead.ownerUserId`, `Prospect.salesOwnerUserId`, `Policy.placedByUserId`,
  `Customer.ownerUserId`) is keyed by `User.id`, not `Employee.id`. `Employee
  PerformanceService` resolves `Employee -> User` via `User.employeeId` before
  computing anything; an employee with no linked User account has nothing to compute
  (404 on the manual trigger, silently skipped by the scheduler's batch).

  **The five metrics, and why each was picked.**

  - **`newClients`** (always a real number, 0 with no activity) — `Customer` rows
    created in the period, attributable via `Customer.prospectId -> Prospect.
    salesOwnerUserId` (the #59 attribution precedent — a Customer onboarded with no
    Prospect isn't attributable to anyone).
  - **`premiumWritten`** (always a real amount) — `SUM(Policy.issuedPremium)` for
    policies this employee PLACED (`Policy.placedByUserId`), issued, with `Policy.
    createdAt` in the period. Unlike #59's caution about premium attribution
    (specifically about crediting a SALES OFFICER — unreliable, since `placedByUserId`
    names the PLACEMENT officer, a different role), crediting WHOEVER PLACED the
    policy is a clean, direct, single-hop relationship — #61 scores employees
    generically, not sales officers specifically, so this dimension works here where
    it didn't for #59. `Policy` carries no dedicated "issued at" timestamp, so
    `createdAt` (when placement was recorded) is the period anchor instead — a
    documented simplification, not an oversight.
  - **`commissionEarned`** (always a real amount) — `SUM(CommissionLedgerEntry.
    amount)` on policies this employee placed, in the period. A plain gross figure,
    no netting against `reversedAmount` — the `kpi-dashboard.md` `commissionThisMonthJod`
    precedent, kept consistent here.
  - **`renewalRatePercent`** (`null` with no denominator, a real 0-100 otherwise) — of
    this employee's placed policies' `RenewalCase`s TRIGGERED in the period that
    reached a terminal outcome (RENEWED/LAPSED/CANCELLED — IN_PROGRESS etc. excluded,
    no outcome decided yet), the proportion RENEWED. `RenewalCase` has NO writer
    anywhere in this codebase today (the renewal module isn't built), so this
    genuinely returns `null` for every employee currently — implemented forward-
    compatible anyway (the #48/#56 dormant-feature precedent) and PROVEN correct by
    seeding `RenewalCase` rows directly via Prisma in the e2e suite (the #56 "plant a
    row to prove the query logic" precedent).
  - **`crossSellRatePercent`** (`null` with no denominator) — of `CrossSellOpportunity`
    rows for customers this employee OWNS (`Customer.ownerUserId`), DETECTED in the
    period, that have been RESOLVED (CONVERTED/DISMISSED — a still-OPEN one is
    excluded, no outcome yet), the proportion CONVERTED. A real, working dimension
    today since Cross-Sell (#8) is fully built.

  **Why the two rate fields are `null`, not `0`, with no denominator.**
  `EmployeePerformanceRecord.renewalRatePercent`/`crossSellRatePercent` are nullable
  columns BY THE SCHEMA'S OWN ORIGINAL DESIGN — unlike #60's `InsurerPerformanceScore`
  (whose four score columns are NOT nullable, forcing a `NEUTRAL_SCORE = 50.00`
  fallback there), #61 can express "no outcomes existed to rate this period" as a
  genuine `null`, distinct from a computed `0%`. `ratePercentOrNull` is the one place
  this rule lives — `newClients`/`premiumWritten`/`commissionEarned` never need this
  distinction, since a count or a sum always has a well-defined zero.

  **The #60 lesson applied proactively, not rediscovered.** `POST /employee-
  performance/compute` requires `employeeId` from the FIRST draft — #60 discovered
  mid-build that an unscoped "recompute everybody" manual trigger breaks against a
  large, e2e-fixture-bloated table. This process started with the `up-sell-
  recommendations/detect`/#60 `insurer-performance/compute` shape already in place:
  the manual trigger names one employee; `EmployeePerformanceService.computeRecords`
  (every linked employee) is called ONLY by `EmployeePerformanceScheduler`, in
  `COMPUTE_CONCURRENCY = 20` bounded chunks via `Promise.allSettled`, never through
  the HTTP layer.

  **Upsert, not append.** `@@unique([employeeId, periodLabel])` (migration
  `20260912120000_add_employee_performance_record_unique`) — both columns always
  non-null, the `InsurerPerformanceScore` shape — makes a recompute UPSERT.

  **Permission.** `employee-performance.view` (`[BRANCH_DEPARTMENT_MANAGER,
  EXECUTIVE_MANAGEMENT]`) — already pre-seeded — gates every route, including the
  manual compute trigger. No new permission was needed, the #60 `insurer-performance.
  view` precedent.

  **A shared utility promoted, not duplicated.** `previousUtcMonthRange`/
  `PeriodWindow` moved from `insurer-performance.config.ts` to new `common/
  period.util.ts` once this process needed the identical calculation —
  `insurer-performance.config.ts` re-exports both so its existing imports keep
  working unchanged (the `calendar-date.util.ts`/`policy.config.ts` promotion
  precedent).

  `apps/web/` gains a new **"Employee performance"** screen
  (`app/(app)/employee-performance/page.tsx` — a lookup form, stat cards + a history
  table with a dash for null rates, and a compute-now form).

  **Verification**: +26 api unit (`period.util.spec.ts` 2 — the shared period-window
  math; `employee-performance.config.spec.ts` 9 — the rate/money view derivations;
  `employee-performance.service.spec.ts` 15 — every metric's computation path, the
  404-with-no-linked-user path, per-employee isolation in the batch, the
  upsert/audit-action split) → api unit **1937** (140 files, from 1911). New
  `test/employee-performance.e2e-spec.ts` **5/5** — permission gating across all
  three routes; a partial-period-override 422; a 404 for an employee with no linked
  User account; the default-previous-UTC-month resolution; a REAL `Customer`/
  `Prospect`/`Policy`×3/`CommissionLedgerEntry`/`RenewalCase`×2/`CrossSellOpportunity`×2
  fixture chain (seeded directly via Prisma) proving all five metrics compute
  correctly — including a genuine 50%/50% renewal and cross-sell rate against real
  Postgres joins — and a same-period recompute upserting instead of duplicating.
  Full api unit suite 1937/1937 confirmed green; full api e2e suite (41 files) green,
  including both documented chronic flakes (`rbac`, `up-sell`) and two confirmed-
  transient environment hiccups (a mid-session system-memory-pressure episode that
  produced multi-hour test runtimes and Prisma "Engine was empty" errors on
  completely unrelated code paths, and a TOTP-timing flake in `auth.e2e-spec.ts` —
  both re-confirmed clean on isolated re-run once the episode passed). New Playwright
  `employee-performance.spec.ts` 4/4; full Playwright suite 159/159 (from 156).
  `npm run typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: #62–65 (Portfolio/Profitability Analysis, Executive Management
  Reporting, Strategic Planning export) remain unbuilt, each with its own permission
  already seeded. No drill-through from a metric to the underlying Customer/Policy/
  CommissionLedgerEntry/RenewalCase/CrossSellOpportunity records that fed it. No real
  `Employee`-onboarding flow exists (Domain H/#66) — `Employee` rows and the `User.
  employeeId` link can only be created directly against the database today, not
  through any HTTP endpoint.

**Part C #62 — Portfolio Analysis (Domain G, Process 62)** — the fifth Domain G
  item: "a query by line/insurer/client segment/geography." The backlog names four
  breakdown dimensions with no fields to back them. Unlike #59–61 (each introducing or
  consuming a dedicated periodic-snapshot model), #62 has **no model of its own at
  all** — a pure, on-demand, current-state read over `Policy` and its joined tables.
  No migration, no scheduler, no new permission: the lightest Domain G process built
  so far.

  **Resolving "line/insurer/client segment/geography" to real fields.** Each
  dimension was mapped to the cleanest existing signal:

  - **line** → `Policy.insuranceLine` (a display-ready string already).
  - **insurer** → `Policy.insurerId`, resolved to `Insurer.name`.
  - **client segment** → `Customer.customerType` (`CORPORATE`/`INDIVIDUAL`) — the
    only clean, existing 2-value segment field anywhere on `Customer`.
  - **geography** → `Branch.name`, resolved via the customer-owning Sales Officer's
    `Customer.ownerUserId -> User.branchId -> Branch.name` — the ONLY structured,
    low-cardinality location-like field anywhere in this schema. `Customer.
    registeredAddress` and `RiskProfile.siteLabel` are both free text and unusable
    for a clean group-by. This is the broker's OWN operational geography (which
    branch serviced the customer), not the customer's physical address.

  All four breakdowns scope to `issuedPremium IS NOT NULL`, the same inclusion rule
  `kpi-dashboard.md`'s `totalIssuedPremiumJod` already uses — a portfolio is a
  current-state snapshot, not a periodic figure like #60/#61.

  **The groupBy-vs-findMany+reduce split.** `insuranceLine`/`insurerId` live directly
  on `Policy`, so `byLine`/`byInsurer` are genuine DB-side `groupBy` calls (the #58
  shape — no read-limit needed, since a `groupBy` result size never scales with row
  count). `Customer.customerType` and the resolved branch name both require crossing
  a relation Prisma's `groupBy` CANNOT cross (it can only group by columns on the
  model being queried), so `byClientSegment`/`byGeography` are instead a capped
  `findMany` (`PORTFOLIO_ANALYSIS_READ_LIMIT = 5000`, the `SLA_DASHBOARD_TIMER_LIMIT`
  shape/value) reduced in pure JS — the `SlaDashboardRepository`/
  `InternalControlsService` "load + reduce + read-limit + truncation warning" shape,
  not #58/#60's pure DB-aggregate shape. An owner with no `branchId` on file resolves
  to `UNASSIGNED_GEOGRAPHY_LABEL` ('Unassigned') rather than being silently dropped.

  **Composition and audit.** `PortfolioAnalysisService.summary()` fires the three
  top-level queries (`groupByLine`, `groupByInsurerId`,
  `findPoliciesForCrossTableGrouping`) via `Promise.all` — the #56 concurrency lesson
  applied from the FIRST draft, not rediscovered via a timing failure. Insurer-name
  and branch-name resolution happen afterward (each needs the ids from the query it
  resolves). A best-effort `READ` audit row is written after the read completes
  (`entityType: 'PortfolioAnalysis'`, `entityId: 'summary'`) — an audit write failure
  never fails the read itself.

  **A shared utility promoted, not duplicated, for the second consecutive process.**
  `formatMoneySum` moved from `kpi-dashboard.config.ts` to `common/money.util.ts`
  once this process needed the identical helper — `kpi-dashboard.config.ts`
  re-exports it so its existing imports keep working unchanged. The same promotion
  #61 did one process earlier for `previousUtcMonthRange`/`PeriodWindow` into
  `common/period.util.ts`.

  **Permission.** `portfolio-analysis.view` (`[BRANCH_DEPARTMENT_MANAGER,
  EXECUTIVE_MANAGEMENT]`) was already pre-seeded — the #60 "one Domain G process
  needing zero seed change" precedent repeated. Gates the single `GET
  /portfolio-analysis` route. `EXTERNAL_AUDITOR` deliberately excluded, the #57
  lesson.

  `apps/web/` gains a new **"Portfolio analysis"** screen
  (`app/(app)/portfolio-analysis/page.tsx` — four breakdown tables, largest-premium
  first).

  **Verification**: +14 api unit (`money.util.spec.ts` +2 for `formatMoneySum`;
  `portfolio-analysis.config.spec.ts` 9 — the pure derivation/reduction logic;
  `portfolio-analysis.service.spec.ts` 5 — composition, concurrency, the truncation
  warning, and the best-effort audit) → api unit **1953** (142 files, from 1937,
  re-confirmed green after `eslint --fix` cosmetic reformatting). New `test/
  portfolio-analysis.e2e-spec.ts` **2/2** — permission gating; a REAL fixture chain
  using uniquely-named `Insurer`/`Branch`/`insuranceLine` values for exact
  `byLine`/`byInsurer`/`byGeography` assertions, plus a before/after DELTA assertion
  on `byClientSegment` (a shared closed-set bucket against the cumulative `db-test`,
  the #58 delta-testing precedent — never a global count). Full api unit suite
  1953/1953 confirmed green; full 42-file api e2e suite green across 8 foreground
  sub-batches, including both documented chronic flakes (`rbac`, `up-sell`)
  re-confirmed passing with `--testTimeout=90000`. New Playwright
  `portfolio-analysis.spec.ts` 3/3; full Playwright suite 161/161 (from 159).
  `npm run typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: #63–65 (Profitability Analysis, Executive Management Reporting,
  Strategic Planning export) remain unbuilt, each with its own permission already
  seeded. No drill-through from a breakdown row to the underlying Policy list.
  `byClientSegment`/`byGeography` are capped at 5,000 policies (a truncation warning
  logs when hit) — `byLine`/`byInsurer` have no such cap. No date-range filter —
  always the whole issued book, not a period.

**Part C #63 — Profitability Analysis (Domain G, Process 63)** — the sixth Domain G
  item: "commission income vs. cost-to-serve per segment/line." Like #62, no new
  model, migration, or scheduler. Unlike #62, this process REUSES the same
  written-policy data #40's `FinancialReportService` already loads, reduced into a
  genuinely different metric.

  **Why this isn't a duplicate of #40's `netPosition`.** `FinancialReportService`
  (#40) already has a "profitability by client segment / line" section:
  `netPosition = premiumWritten − claimsPaid − commissionEarned` — an
  underwriting-result view of the INSURER's side of the book. The backlog's own
  phrase for #63 asks a different question: is the commission the BROKER earns on
  this book enough to cover what it costs the broker to service it? That is
  `netProfitability = commissionIncome − costToServe`, not `premiumWritten −
  claimsPaid − commissionEarned`. Both read the identical raw policy/claims/
  commission data; they reduce it differently, for different audiences.
  ...
  - **`commissionIncomeJod`** — Σ (`commissionAmount − commissionReversedAmount`)
    ...
    this process needs the precise figure since it IS the metric.
  - **`costToServeJod`** — Σ net settlement of the group's SETTLED / CLOSED
    claims. A DOCUMENTED SCOPED INTERPRETATION, not a true operational cost: no
    operational-expense / staff-time / overhead tracking model exists anywhere in
    this schema (Domain H / backlog #66, HR, is not built) — claims-settlement
    payout is the one real cost-shaped figure attributable to a line/segment
    anywhere in the schema, so it stands in as the closest available signal. This
    is the SAME `claimsPaid` figure #40's `netPosition` already computes.
  - **`netProfitabilityJod`** = `commissionIncomeJod − costToServeJod`. Can be
    negative (a segment costing more in claims churn than it earns in commission).

  Grouped by `insuranceLine` and `customerType` ONLY — the backlog names exactly
  these two dimensions, unlike #62's four (no insurer, no geography). Rows sort
  worst-first (most-negative `netProfitabilityJod`), the #40 `groupProfitability`
  convention.

  **A new kind of promotion, not a pure-function one.** #61/#62 each promoted a
  small PURE utility function (`previousUtcMonthRange`, `formatMoneySum`) once a
  second consumer needed it. #63 applied the same PATTERN to something bigger: a
  non-trivial Prisma QUERY. `ProfitabilityPolicyRow` and its loader
  (`loadProfitabilityPolicies`, a `findMany` joining `Policy` -> `Customer`/
  `Claim`/`Settlement`/`CommissionLedgerEntry`) were extracted OUT of
  `FinancialReportRepository` (#40) into a new standalone
  `repositories/profitability-policy.repository.ts`
  (`ProfitabilityPolicyRepository.loadWrittenPolicies(limit)`) —
  `finance.config.ts` re-exports the `ProfitabilityPolicyRow` type so its existing
  imports keep working unchanged, the #61/#62 re-export courtesy.

  **Module wiring stayed zero-coupling.** `ProfitabilityPolicyRepository` is
  provided independently in BOTH `FinanceModule`'s and the new
  `ProfitabilityAnalysisModule`'s own `providers` arrays — the SAME class, two
  separate instances, both wrapping the singleton `PrismaService`. No
  `imports`/`exports` wiring was added to either module — `kpi-dashboard.md`'s "no
  cross-module SERVICE dependency" precedent held even while sharing the
  underlying QUERY.

  **Permission — a real deviation from #58-62's pattern.**
  `profitability-analysis.view` (already pre-seeded) grants `[EXECUTIVE_MANAGEMENT,
  FINANCE_COLLECTIONS_OFFICER]` — NOT `BRANCH_DEPARTMENT_MANAGER`, unlike every
  #58-62 Domain G permission. Zero seed change, the #60/#62 "no new permission"
  precedent repeated a third time. `EXTERNAL_AUDITOR` deliberately excluded, the
  #57 lesson.

  The `costToServe` figure aggregates HIGHLY_CONFIDENTIAL `Claim` settlement data
  whenever a settled claim contributed — the best-effort READ audit row flags
  `isSensitiveDataAccess: summary.totals.claimCount > 0`, the exact #40
  `recordReadBestEffort` rule.

  `apps/web/` gains a new **"Profitability analysis"** screen
  (`app/(app)/profitability-analysis/page.tsx` — two breakdown tables: by line, by
  client segment).

  **Verification**: +13 api unit (`profitability-analysis.config.spec.ts` 8 — the
  pure commission-income/cost-to-serve/net-profitability reduction and sort;
  `profitability-analysis.service.spec.ts` 5 — composition, the truncation
  warning, and the sensitive-data audit rule) → api unit **1966** (144 files, from
  1953). The #40 extraction was re-verified transparent throughout:
  `financial-report.service.spec.ts`/`finance.config.spec.ts` 55/55 unchanged,
  re-confirmed again after `eslint --fix` cosmetic reformatting. New `test/
  profitability-analysis.e2e-spec.ts` **3/3** — permission gating; an explicit
  assertion that Branch/Department Manager is FORBIDDEN (documenting the
  deliberate role-scope difference from #58-62); a REAL fixture chain (a
  uniquely-named `insuranceLine` + a SETTLED `Claim`/`Settlement` + a
  `CommissionLedgerEntry`, seeded directly via Prisma) proving
  `commissionIncomeJod`/`costToServeJod`/`netProfitabilityJod` compute correctly,
  plus a before/after DELTA on `bySegment` (a shared closed-set bucket against the
  cumulative `db-test`, the #62 precedent). Full api unit suite 1966/1966
  confirmed green; full 43-file api e2e suite green across 8 foreground
  sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing with
  `--testTimeout=90000`, including #40's OWN e2e coverage (inside
  `invoice.e2e-spec.ts`) re-confirmed green after the extraction. New Playwright
  `profitability-analysis.spec.ts` 3/3; full Playwright suite 207/207.
  `npm run typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: #64–65 (Executive Management Reporting, Strategic Planning export)
  remain unbuilt, each with its own permission already seeded. No drill-through
  from a breakdown row to the underlying Policy/Claim list. `costToServeJod` is a
  scoped claims-payout proxy, not a true operational cost — would need Domain H /
  #66 (HR) or a new expense-tracking model to become one. No date-range filter —
  always the whole written book, not a period.

**Part C #65 — Strategic Planning Inputs (Domain G, Process 65)** — **the last
  Domain G item built; #64 Executive Management Reporting deliberately not built
  yet**, per the user's own pacing through this backlog. The line: "export
  portfolio/market data for planning cycles." No new model, migration, or
  scheduler — this process composes TWO already-built reports:

  - **portfolio** — the SAME four breakdowns #62 (`PortfolioAnalysisService.
    summary()`) already computes: byLine/byInsurer/byClientSegment/byGeography,
    a current-state book snapshot with no period.
  - **market** — every insurer's `InsurerPerformanceScore` (#60) for ONE period
    — quote-response speed/claims service/price/service quality, the closest
    existing signal for "the state of the market this broker places business
    with." Defaults to the previous UTC calendar month via `common/
    period.util.ts`'s `previousUtcMonthRange` — a third consumer of that shared
    helper, after #60 and #61.

  **The first genuinely new capability shape in this codebase.** No CSV/
  file-download precedent existed anywhere before this process (`grep`-confirmed
  — no `Content-Disposition`, `text/csv`, or `StreamableFile` usage anywhere in
  `apps/api/src`), and none was introduced here either: the response stays a
  plain JSON payload, matching the convention every other report in this
  codebase already uses. "Export" means composing a structured snapshot for an
  external planning tool to consume, not producing a downloadable file.

  **This codebase has a single, zero-exception, VERIFIED cross-module rule.**
  Inspecting every cross-module `import` under `apps/api/src/modules/` confirms
  it: a module that needs another domain's data imports that OTHER module for
  its REPOSITORY export ONLY, never its SERVICE — `CustomerModule` exports only
  `CustomerRepository` (never `CustomerService`), and `PolicyModule`/
  ...
  independently, by two different modules (`ProfitabilityPolicyRepository`).
  #65 needed the SAME treatment for TWO repositories at once —
  `PortfolioAnalysisRepository` (#62) and `InsurerPerformanceRepository` (#60)
  — both provided independently in `PlanningExportModule`'s own `providers`
  array, with ZERO changes to `PortfolioAnalysisModule` or
  `InsurerPerformanceModule`. `PlanningExportService` reuses their exported PURE
  derivation functions directly (`deriveLineOrInsurerBreakdown`/
  ...
  config.ts`) — the actual reduction logic is never duplicated, only the thin
  `Promise.all` + name-resolution orchestration glue every composing report in
  this codebase (e.g. #40's own `FinancialReportService.summary()`) already
  repeats for itself. Injecting `PortfolioAnalysisService`/
  `InsurerPerformanceService` directly was considered and rejected — it would
  have been the FIRST exception to a rule with zero exceptions across 65 built
  processes.

  **Permission — the narrowest Domain G grant.** `planning-export.generate`
  (already pre-seeded) grants `[EXECUTIVE_MANAGEMENT]` ONLY — no
  `BRANCH_DEPARTMENT_MANAGER` (unlike #58-62), no `FINANCE_COLLECTIONS_OFFICER`
  (unlike #63's own deviation). Zero seed change. `EXTERNAL_AUDITOR`
  deliberately excluded, the #57 lesson. A POST route (`POST /planning-export`),
  not a GET, even though it is a pure read with no persisted side effect of its
  own — the permission's own verb ("generate") matches the `internal-controls.
  audit` "Run audit now" shape more than the `.view` shape #58-63 use.

  **The first real writer of `AuditAction.EXPORT`.** This enum value has
  existed in the core schema since before this session's work began, and
  `AuditAnomalyDetectionService.checkBulkExport` (Part 10.3) has ALREADY been
  checking for it — flagging a `BULK_EXPORT` `AccessAnomalyAlert` whenever one
  user exceeds `BULK_EXPORT_THRESHOLD` EXPORT actions in a trailing
  `BULK_EXPORT_WINDOW_MINUTES` window. No process before #65 had ever written an
  `EXPORT` audit row. `AuditService.record()` already calls
  `anomalyDetection.evaluate()` automatically after every write, so
  `PlanningExportService`'s own best-effort `EXPORT` audit call activated this
  dormant Part 10.3 detector with ZERO extra wiring — the same "dormant,
  forward-compatible feature, proven by its first real writer" pattern as
  #48/#56/#60/#61's own findings.

  `apps/web/` gains a new **"Strategic Planning Inputs"** screen
  (`app/(app)/planning-export/page.tsx` — a form-triggered generate action, not
  an auto-loading dashboard, the #60/#61 "compute now" form shape since this is
  a POST action, not a GET read).

  **Verification**: +9 api unit (`planning-export.config.spec.ts` 3 — the pure
  period-resolution helper; `planning-export.service.spec.ts` 6 — composition,
  concurrency, the truncation warning, and the audit rule) → api unit **1975**
  (146 files, from 1966). New `test/planning-export.e2e-spec.ts` **4/4** —
  permission gating; an explicit assertion that BOTH
  `BRANCH_DEPARTMENT_MANAGER` AND `FINANCE_COLLECTIONS_OFFICER` are FORBIDDEN
  (proving the narrowest-grant claim); the default-previous-UTC-month
  resolution; a REAL fixture (a uniquely-named `insuranceLine`/`Insurer` + a
  real `InsurerPerformanceScore` row, seeded directly via Prisma) proving
  portfolio + market data compose correctly for an explicit period. Full api
  unit suite 1975/1975 confirmed green; full 44-file api e2e suite green across
  8 foreground sub-batches — two confirmed-transient environment hiccups this
  run (a `sales-performance` isolated re-run, and a `risk-profile`/`security`/
  `service-request`/`sla-dashboard`/`transaction-monitoring` batch re-run),
  both unrelated files re-confirmed clean; both documented chronic flakes
  (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright
  `planning-export.spec.ts` 3/3; full Playwright suite 210/210.
  `npm run typecheck`/`lint`/`build` (api + web) OK — one real lint catch this
  time: `@typescript-eslint/no-unsafe-assignment` on an asymmetric Vitest
  matcher (`expect.stringMatching`) nested inside a `toHaveBeenCalledWith`
  object literal, fixed by asserting the captured mock-call argument directly
  instead of matching inside the call-args object.

  **Deferred**: #64 (Executive Management Reporting) remains unbuilt, its own
  permission already seeded. No actual downloadable file (CSV/XLSX) — this
  stays a JSON API response, consistent with every other report in this
...
**Part C #68 — Internal Information Technology (Domain H, Process 68)** —
  **verified, not built.** The backlog's own annotation: "Also only a general
  description ('system administration, change management, environment
  management') with no field-level detail. The tasks actually executable
  from the source already exist under A.10 (environment separation) and Part
  10.5 (change management with security sign-off before deployment) — no
  standalone data table is required by the text." This process did not take
  that claim at face value — it checked the real current state of both
  citations before accepting "no build" as the right outcome.

  **"No standalone data table" holds up.** #68's description names system
  administration / environment / change management, not a business-entity
  concern — there is no schema gap a new module could meaningfully close, so
  this is a genuine `#47`/`#50` "verified-covered" shape, not a build that
  got skipped.

  **But the coverage claim is overstated on the other two counts.**
  (1) A.10's own "Separate Dev/Test/UAT/Prod environments" checkbox is
  self-documented in THIS SAME README's § Known gaps, A.10, as "scaffolded,
  not achieved" — `db`/`db-test`/`db-uat` are three local docker-compose
  Postgres instances with no real UAT/Prod *deployment* target, so "UAT"
  today means a fourth local database, not a reachable separate environment.
  (2) **"Part 10.5" as a distinct "change management with security sign-off"
  control has no independent definition anywhere in this repo or
  `ibms-brain`** — grepped exhaustively across both, it appears nowhere
  except inside this #68 annotation itself. It is the backlog author's own
  gloss on A.10's combined "Part 10.4/10.5" citation, not a quote from a
  defined section. The closest real artifact — DAST in CI
  (`.github/workflows/ci.yml`'s ZAP baseline step) — is explicitly
  informational-only (`fail_action: false`, § Known gaps A.10), and this
  repo has no deployment pipeline at all (`.github/workflows/` holds only
  `ci.yml`/`codeql.yml`/`backup-drill.yml`) for a "sign-off before
  deployment" gate to attach to.

  **Verdict**: no separate build was made — #68 names no new business-entity
  concern a schema/CRUD module could address, so this verification pass IS
  the process's own completion. But its coverage should be read as "the same
  infra gaps A.10 already tracks," not as "done."
  ...
  **Deferred**: everything already listed under § Known gaps, A.10 (no
  ...
  stays three local databases). Nothing #68-specific beyond that. #69
  Cybersecurity, #70 Document Management, #71 Vendor Management, #72-73
  BCP/DR, and #74 Knowledge Management (the next five entries below) are
  now built, closing Domain H.
...
**Part C #69 — Cybersecurity (Domain H, Process 69)** — **verified AND
  ...
  **Deferred**: Part A's own gaps (hardware-token MFA, a real KMS/HSM,
  ...
  by #69's own text, not built here. #70 Document Management, #71 Vendor
  Management, #72-73 BCP/DR, and #74 Knowledge Management (the next four
  entries below) are now built, closing Domain H.

**Part C #70 — Document Management (Domain H, Process 70)** — full version
  control (previous/next version) + mandatory classification + a deletion
  lock disabled by default except via a logged privileged override
  (`deletionOverrideByUserId`); plus enforcing the "highest classification
  present" rule when a file/record mixes classification levels.

  **`Document` (Part 4.2, the electronic Insurance File) pre-exists with
  THREE prior writers** — Policy issuance/attach (#18-19), Claim
  ...
  this process — `audit-trail.repository.ts`'s own
  `findDocumentVersionChain` doc comment already said as much ("no
  application code creates a second version yet"). Notably, a READ-side
  capability (#57's External Auditor document-history lens) had existed for
  LONGER than any writer — the version-chain walker was built before
  anything ever populated a second link in the chain. #70 is that first
  real writer.

  **Version-chain creation mirrors `QuotationService.revise` exactly.**
  The target `id` must be the chain's own CURRENT (leaf) version — passing
  a superseded id 422s ("create a new version from its own latest version
  ...
  code `P2002`). A new version's `category`/`policyId`/`customerId` are
  inherited from its predecessor, never accepted from the caller — a
  version is a revision of the SAME logical document, not a new artifact.
  Only `fileName`/`storageRef`/`classification` can change per version, and
  `classification` stays mandatory on every version (not just the first) —
  the Part 10.6 "the officer must say which, never assume" reasoning
  `create-customer-document.dto.ts` already established for version 1.

  **Deletion is a single-actor privileged override, NOT the M06 Disposal
  dual control.** `roles-and-segregation-of-duties.md` requires dual
  control — Department Manager sign-off plus DPO final approval — for the
  *disposal BATCH* destruction decision (`DisposalBatch`, a separate,
  still-unbuilt process, already flagged `dormant: true` in
  `internal-controls.config.ts`'s `MAKER_CHECKER_REGISTRY`). #70's own
  field is singular (`deletionOverrideByUserId`), and its pre-seeded
  permission is a single code, not two distinct maker/checker codes
  (`document.delete-override`, `[SYSTEM_SECURITY_ADMINISTRATOR,
  DATA_PROTECTION_OFFICER]`) — both anticipate the narrower, ad hoc,
  single-document shape instead of the batch-level dual control. This was
  checked against the actual seed data and `maker-checker-segregation.md`'s
  own maker/checker table before assuming a two-actor pattern was needed.

  **A document can only be deleted from the leaf of its own version
  chain — an APPLICATION-level guard, not a DB one.**
  `Document_previousVersionId_fkey` is `ON DELETE SET NULL`, not
  `RESTRICT` — Postgres will not stop a delete that orphans a successor,
  it will silently null the successor's `previousVersionId` instead,
  breaking the chain. `hasSuccessor(id)` is checked before every delete; a
  document still linked to a `ClaimDocument` also cannot be deleted, and
  there the DB's own `ON DELETE RESTRICT` on `ClaimDocument.documentId` IS
  a genuine backstop behind the same app-level pre-check. Hardening the
  `previousVersionId` FK to `RESTRICT` was considered and deferred — not
  asked for by #70's own two checkboxes, a documented, deliberate
  limitation rather than an oversight. `overrideDeletionLock`/
  `deleteIfUnlocked` are both status-conditional writes (`updateMany`/
  `deleteMany` re-asserting `deletionLocked` in the `where`), never a
  `findUnique` followed by an unconditional write — a second concurrent
  override 409s instead of silently overwriting who is on record for it.

  **The "highest classification present" rollup is scoped to
  `Document.policyId` only** (`GET /documents/classification-summary?
  policyId=`), matching the schema's own "every Policy resolves to ONE
  electronic Insurance File" framing. A claim-linked document (joined via
  `ClaimDocument`, never carrying its own `policyId`) is deliberately out
  of scope — a literal reading of what the source text anchors the rule
  to, not a gap. The rule itself is `sensitive-data-handling.md` /
  ...
  **`#70`'s new `document.manage` routes deliberately do not duplicate
  `#57`'s existing `GET /audit-trail/documents/:id/history`**
  (`document-history.read`, `[COMPLIANCE_OFFICER, EXTERNAL_AUDITOR]` only —
  a compliance/forensics lens combining the version list with
  `AuditLogEntry` rows). The people who actually upload/version documents
  (`document.manage`: Sales/Placement/Claims/Finance/Compliance) mostly
  cannot reach that route at all — a Placement officer versioning a policy
  document has no read access to `document-history.read`. #70's own
  `GET /documents`/`GET /documents/:id` are a plain OPERATIONAL browse
  ("what documents exist right now"), not a forensic view — a deliberate,
  non-overlapping addition.

  **No new permission, no migration** — `document.manage`/
  `document.delete-override` were both pre-seeded ahead of time, Domain
  H's "seed before code" pattern holding again after #69 broke it.

  **New module** `apps/api/src/modules/supporting-operations/document.
  {config,service,controller,module}.ts` + `repositories/document.
  repository.ts`. `apps/web/` gains a **"Documents"** screen
  (`app/(app)/documents/page.tsx` — a policy-scoped lookup, per-row
  version/unlock/delete actions, and a classification-summary lookup).

  **Verification**: +21 api unit (`document.config.spec.ts` 6,
  `document.service.spec.ts` 15) → api unit **2044** (152 files, from
  2023). New `test/document.e2e-spec.ts` **6/6** — permission gating
  (`document.manage` vs `document.delete-override` checked separately, not
  as one grant); create-version → 422 on a superseded id; the full
  lock/unlock/delete lifecycle (409 while locked, unlock, a second override
  409s, delete succeeds, the row confirmed gone via a direct Prisma read
  since the deleting ADMIN role holds no read permission of its own); a
  409 deleting a document with a newer version even once unlocked; a 409
  deleting a document still attached to a claim; the classification-summary
  rollup for a real Policy fixture + a 404 for an unknown policy. Full api
  unit suite 2044/2044 confirmed green; full 48-file api e2e suite green
  across 8 foreground sub-batches — one confirmed-transient environment
  hiccup (an unrelated file's Prisma engine blip) and one confirmed-
  transient TOTP-timing flake (also an unrelated file) this run, both
  re-confirmed clean on isolated re-run, both documented chronic flakes
  (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright
  `documents.spec.ts` 5/5; full Playwright suite 227/227 (from 222).
  `npm run typecheck`/`lint`/`build` (api + web) OK — two real lint
  catches: unescaped apostrophes (`react/no-unescaped-entities`) in the new
  web page's prose, and a TypeScript literal-type inference gap in the new
  Playwright spec's mutable fixture object (a `null`-typed field needing
  `as string | null`), both fixed.

  **Deferred**: hardening `Document_previousVersionId_fkey` from
  `ON DELETE SET NULL` to `RESTRICT` is a real, documented follow-up, not
  built here. The M06 Disposal Management dual-control workflow
  (`DisposalBatch`, Department Manager + DPO) remains a separate,
  still-unbuilt process. `#57`'s External Auditor document-history lens is
  unchanged, still the compliance-forensics view. A new version's
  classification is validated against the full `DataClassification` set
  regardless of whether the chain is customer-linked —
  `create-customer-document.dto.ts`'s own narrower CONFIDENTIAL/
  HIGHLY_CONFIDENTIAL-only restriction is a create-time-only rule, not
  re-derived for later versions. #71 Vendor Management, #72-73 BCP/DR, and
  #74 Knowledge Management (the next three entries below) are now built,
  closing Domain H.

**Part C #71 — Vendor Management (Domain H, Process 71 — M07)** — risk
  tiering (low/medium/high) before any data share or access grant; a
  mandatory Data Processing Agreement for Medium/High tier with additional
  DPO approval for High tier before the first share; mandatory annual
  review + confirmation of data return/destruction on termination + access
  revocation within 2 business days.

  **#67 (Procurement) built the foundational `Vendor` CRUD and explicitly
  deferred `riskTier`/`annualReviewDueAt`/`terminationDataReturnConfirmedAt`/
  `accessRevokedAt` to this process** — its own module doc comment already
  said so. This process extends the SAME `vendor.{config,service,
  controller,repository,module}.ts` files rather than building a parallel
  Vendor CRUD.

  **Checkbox 1 ("risk tiering ... before any data share or access grant")
  has no live action to enforce against.** `DataSharingApproval` (M08) has
  zero prior writer anywhere in this codebase (flagged `dormant: true` in
  `internal-controls.config.ts`'s `MAKER_CHECKER_REGISTRY`, the same shape
  `DataProcessingAgreement` was in before this process), and no model
  represents a generic "access grant" at all. Rather than invent the M08
  workflow, this process built `computeDataShareReadiness()`
  (`vendor.config.ts`) — a pure, queryable rule
  (`GET /vendors/:id/data-share-readiness`) — an honest, forward-compatible
  gate with NO live enforcement call site yet, not a claim that anything is
  actually blocked today.

  **`DataProcessingAgreement`'s maker/checker pair already had a DB `CHECK`
  constraint and a covered-pairs table entry since the A.5 foundational
  work, with zero prior application writer.** This process is that first
  real writer. `dpa.approve` (DPO only) was already pre-seeded as a
  DISTINCT permission code from `vendor.manage` — the maker-checker
  default (two codes), not the `incident.classify` shared-permission
  exception.

  **This is NOT the M06 Disposal dual control.** `roles-and-segregation-
  of-duties.md` and `maker-checker-segregation.md` both describe dual
  control (Department Manager + DPO) for "data deletion of an Insurance
  File record" / the *disposal BATCH* destruction decision — that is the
  M06 `DisposalBatch` workflow, a separate, still-unbuilt process. This
  process's own DPA maker/checker pair is a different, already-named pair
  in the maker/checker table, not a stand-in for M06.

  **`vendor_termination_access_revocation` (2 business days) needed a
  genuinely NEW SLA registry entry.** Unlike this SAME process's own
  `vendor_annual_review` (already pre-seeded with zero prior caller, the
  #60/#61/#66 dormant-SLA shape), `pdpl-sla-timers.md`'s lex table had NO
  row at all for vendor termination access revocation — a real gap, not a
  dormant-but-present one. Both the lex table and `sla-registry.config.ts`
  needed a new row, sourced directly from this backlog line's own "2
  business days" text (the M03 consent-withdrawal precedent for sourcing
  an SLA value straight from the backlog).

  **`Vendor` has no explicit "terminated" status field.** Termination is
  represented by `terminationDataReturnConfirmedAt` going null-to-set (the
  #66 `Employee.terminationDate` shape), gated by a mandatory explicit
  `confirmDataReturnOrDestruction: true` attestation (`TerminateVendorDto`,
  checked in the service — the `FulfilDsrDto` staff-attestation
  precedent) since there is no automated verification that data was
  actually returned or destroyed. `accessRevokedAt` is a separate,
  subsequent stamp requiring termination to have happened first (422
  otherwise) — two independently status-conditional actions (409 on
  double-execution), not one compound action.

  **Risk tiering auto-schedules the annual-review SLA on the FIRST
  Medium/High tiering only** — a later downgrade to Low does not
  un-schedule an already-scheduled review, a deliberate simplification.
  `recordAnnualReview()` re-bases the schedule +12 months by resolving the
  old `SlaTimer` row(s) and starting a new one (the documented re-basing
  shape) — 422s if no review is currently scheduled.

  **No new permission, no migration** — `vendor.manage` and `dpa.approve`
  were both already pre-seeded.

  **New files**
  `apps/api/src/modules/supporting-operations/data-processing-agreement.
  {service,controller}.ts` + `repositories/data-processing-agreement.
  repository.ts`, registered in the SAME `vendor.module.ts`. `apps/web/`
  gains a **vendor detail screen**
  (`app/(app)/vendors/[id]/page.tsx` — risk tiering, readiness check, DPA
  lifecycle, termination/revocation); the list page gains a "Manage" link
  into it.

  **Verification**: +42 api unit (`vendor.config.spec.ts` 13,
  `vendor.service.spec.ts` rewritten to 27 tests, `data-processing-
  agreement.service.spec.ts` 10, plus an `sla-registry.config.spec.ts`
  count-assertion update for the new 15th PDPL-sourced workflow) → api
  unit **2086** (154 files, from 2044). Extended `test/vendor.e2e-spec.ts`
  to **8/8** (from 4) — permission gating unchanged; a full
  risk-tiering→annual-review→re-review walk; the full DPA maker/checker
  lifecycle + readiness gate for a High-tier vendor (a 403 self-approval,
  a 409 double-approval); Low-tier readiness with no DPA at all; the
  termination→revocation walk with order-guarding (422 revoking before
  terminating) and double-execution 409s. Full api unit suite 2086/2086
  confirmed green; full 48-file api e2e suite green across 8 foreground
  sub-batches, both documented chronic flakes (`rbac`, `up-sell`) passing
  with `--testTimeout=90000`. New Playwright `vendor-detail.spec.ts` 5/5;
  full Playwright suite 232/232 (from 227). `npm run typecheck`/`lint`/
  `build` (api + web) OK — one real accessibility catch (an unlabeled
  risk-tier `<select>`), fixed with a wrapping `<label>`.

  **Deferred**: M08 (`DataSharingApproval`) remains a separate,
  still-unbuilt PCMS module — `computeDataShareReadiness()` is built for
  it to eventually call, not a substitute for building it. M06
  (`DisposalBatch`) remains a separate, still-unbuilt batch-destruction
  dual-control workflow. The `vendor_termination_access_revocation` SLA
  figure has no independent `PRIV-SOP-10` citation beyond the backlog's
  own "2 business days" text, the same documented-sourcing-gap shape as
  `CLAIM_LARGE_THRESHOLD_JOD`. #72-73 BCP/DR and #74 Knowledge Management
  (the next two entries below) are now built, closing Domain H.

**Part C #72-73 — Business Continuity & Disaster Recovery (Domain H,
  Process 72-73)** — a plan for every scenario the source names explicitly
  (system outage, site/office loss, cyberattack/ransomware, key-staff
  unavailability, insurer-side service interruption); a documented
  RTO/RPO + last-tested date + next-test-due date per plan.

  **`BcpDrPlan` (schema's own doc comment: "Process 72-73 — continuity/DR
  plans and tests") pre-exists with zero prior application code** — the
  same "dormant model, first real writer" shape #58-71 repeatedly found.
  `bcp-dr.manage` (`[SYSTEM_SECURITY_ADMINISTRATOR, COMPLIANCE_OFFICER]`)
  was already pre-seeded — no seed change needed.
  ...
  **`meta/lex/backup-rpo-rto.md` already covers ONE narrow, already-tested
  ...
  drill (`.github/workflows/backup-drill.yml`, weekly, a real pass/fail on
  row-count parity + a timed RTO check). That lex file's own RPO/RTO
  figures (24h/15min) are DRAFT, database-specific, and pre-date this
  process — not restated or duplicated here. The other four named
  scenarios are organizational/procedural plans with NO code-level
  automation anywhere in this repo — `BcpDrPlan` is a plain record of a
  plan's existence and test history, not a system that executes or
  verifies the plan itself.
  ...
  **No sourced test-cadence figure exists for BCP/DR plans generally.**
  ...
  in `pdpl-sla-timers.md`), nothing names how often a plan must be
  re-tested — `nextTestDueAt` is therefore a plain CALLER-SUPPLIED field
  on `recordTest()` (`RecordBcpDrPlanTestDto.nextTestDueAt`, mandatory),
  never auto-computed from a fabricated interval. This process is
  deliberately NOT wired into `SLA_REGISTRY` at all — that registry is for
  PDPL-sourced SLAs `pdpl-sla-timers.md` tracks (M03-M10 business rules);
  BCP/DR testing cadence is a CBJ operational-resilience concern (Part
  10.4/10.5), a different regulatory domain entirely.

  **Checkbox 1 is answered by a genuine coverage/gap check, not just a
  CRUD a caller must trust was populated correctly.**
  `GET /bcp-dr-plans/coverage` (`computeScenarioCoverage()`) returns all
  five named scenarios every time, each with its own `plans` array
  (possibly empty) and a `hasPlan` flag — the #59/#69 "verify coverage,
  don't just build a CRUD and assume backlog intent is satisfied"
  discipline applied to a five-item checklist instead of a single field.

  **`planDocumentId` is validated against a real `Document`** via
  `DocumentRepository.findById()` in the service layer (404 if not found)
  — the #66/#69 bare-scalar link-validation precedent, now reachable
  because #70 (Document Management) built real `Document` CRUD for this
  process to validate against. `scenario` is immutable once a plan is
  created — reclassifying it would just be a new plan for a different
  scenario.

  **New module**
  `apps/api/src/modules/supporting-operations/bcp-dr-plan.
  {config,service,controller,module}.ts` +
  `repositories/bcp-dr-plan.repository.ts`. `apps/web/` gains a
  **"BCP / DR plans"** screen (`app/(app)/bcp-dr-plans/page.tsx` — the
  coverage view with gaps flagged in red, a create form, a per-plan
  record-test action).

  **Verification**: +16 api unit (`bcp-dr-plan.config.spec.ts` 6,
  `bcp-dr-plan.service.spec.ts` 10) → api unit **2102** (156 files, from
  2086). New `test/bcp-dr-plan.e2e-spec.ts` **6/6** — permission gating;
  a 400 rejecting a scenario outside the documented 5-value set; a 404
  creating a plan with a `planDocumentId` that doesn't reference a real
  `Document`; a real create→list(filtered)→get→update→record-test walk
  with a REAL linked `Document` fixture (scenario proven immutable across
  the update); 404s for an unknown plan on both GET and PATCH; the
  five-scenario coverage view with a real gap-vs-covered assertion. Full
  api unit suite 2102/2102 confirmed green; full 49-file api e2e suite
  green across 7 foreground sub-batches, both documented chronic flakes
  (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright
  `bcp-dr-plans.spec.ts` 5/5; full Playwright suite 237/237 (from 232).
  `npm run typecheck`/`lint`/`build` (api + web) OK.

  **Deferred**: the database backup/restore drill remains the one piece
  of real infrastructure automation touching any of the five scenarios —
  the other four have no code-level backing. Any future automated BCP/DR
  test-cadence SLA would need a real sourced figure (a `PRIV-SOP`/CBJ
  circular citation) before joining `SLA_REGISTRY`, not invented here.
  #74 (Knowledge Management, the next entry below) is now built, closing
  Domain H.

**Part C #74 — Knowledge Management (Domain H, Process 74 — closes Domain
  H)** — a bilingual knowledge base: product knowledge, insurer appetite,
  rate guides, regulatory updates.

  **`KnowledgeBaseArticle` (schema's own doc comment: "Process 74 —
  product knowledge, insurer appetite, rate guides, regulatory updates for
  staff") pre-exists with zero prior application code** — the same
  "dormant model, first real writer" shape #58-73 repeatedly found,
  closing out Domain H on the same pattern it opened with. `kb.publish`
  (`[COMPLIANCE_OFFICER, BRANCH_DEPARTMENT_MANAGER,
  ...
  **"Bilingual" here is OPTIONAL-per-article, not mandatory-both.** The
  sibling `DocumentTemplate` model (Part 11.2 — regulator-facing formal
  documents) has `nameEn`/`nameAr`/`bodyEn`/`bodyAr` ALL `NOT NULL`;
  `KnowledgeBaseArticle` is the opposite — only `title` is mandatory,
  `titleAr`/`bodyEn`/`bodyAr` are all nullable, so an article may exist in
  English only, Arabic only, or both. This is a deliberate reading of the
  schema's own nullability, not something to "fix" by making the Arabic
  fields mandatory just because the backlog line says "bilingual."

  **Creation IS publishing** — `publishedAt` defaults to `now()` at the DB
  level with no nullable "draft" state, matching the pre-seeded
  permission's own verb (`kb.publish`, not `kb.manage`/`kb.create`).
  `POST /knowledge-base-articles` never accepts a caller-supplied
  `publishedAt`.
  ...
  **`kb.publish` gates the WHOLE surface (create/list/get/update), not
  ...
  gates the whole CRUD" precedent applied again, with a real, documented
  consequence: only that permission's own three roles can even READ the
  knowledge base via this API, since no second, broader read permission
  was pre-seeded. A genuinely useful company-wide knowledge base would
  likely want much wider read access — this is a real, deliberate scope
  limit of the backlog's own permission grid, not solved by inventing a
  ...
  **`category` is mutable via `PATCH`, unlike #72-73's `BcpDrPlan.scenario`
  ...
  fixing metadata, not rewriting a scenario's own history; the same domain
  can make different mutability calls for a similarly-shaped "classifying
  dimension" field depending on what it actually represents. No
  maker/checker, no SLA timer, no cross-entity FK validation — the
  ...
  **New module**
  ...
  {config,service,controller,module}.ts` +
  `repositories/knowledge-base-article.repository.ts`. `apps/web/` gains a
  **"Knowledge Base"** screen (`app/(app)/knowledge-base/page.tsx` — a
  bilingual list with RTL-aware Arabic columns/inputs, a publish form,
  inline title/titleAr edit).

  **Verification**: +9 api unit (`knowledge-base-article.service.spec.ts`)
  → api unit **2111** (157 files, from 2102). New
  `test/knowledge-base-article.e2e-spec.ts` **5/5** — permission gating; a
  400 rejecting a category outside the documented 4-value set; a real
  create(English-only)→list(filtered)→get→update(add Arabic translation)
  walk; a fully bilingual create in one call; 404s for an unknown article
  on both GET and PATCH. Full api unit suite 2111/2111 confirmed green;
  full 50-file api e2e suite green across 8 foreground sub-batches, both
  documented chronic flakes (`rbac`, `up-sell`) passing with
  `--testTimeout=90000`. New Playwright `knowledge-base.spec.ts` 5/5; full
  Playwright suite 242/242 (from 237). `npm run typecheck`/`lint`/`build`
  (api + web) OK.

  **Deferred**: any broader, company-wide read permission for the
  knowledge base — a real, documented gap, not solved here since the
  backlog's own pre-seeded grid names only `kb.publish`. `DocumentTemplate`
  (Part 11.2) itself remains a separate, still-unbuilt model for
  regulator-facing bilingual formal documents. **This closes Domain H
  (Supporting Operations, backlog #66-74) entirely** — #66 Human Resources,
