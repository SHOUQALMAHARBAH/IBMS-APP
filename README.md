...
### Part C · Domain D #31–33 — built, with these deferrals
...
- **Domains C–H** — Claims **#23–30 are built** (Notification, Registration, Documentation,
  ...
  Client Accounting (#33) are built** (see the Domain D table above — the full
  `Invoice → Receipt → Reconciliation → Remittance` cycle through the workflow engine,
  with a client-funds ledger entry at each money movement, plus the on-the-fly
  accounts-receivable / ageing report per customer); insurer accounting / commission /
  bank-reconciliation / reporting (#34–40),
  Customer
