...
- **`@code-reviewer` pass** (mandatory — financial calculation: premium / deductible /
  ...
  Carried follow-up: the RFQ follow-up sweep is unaware of the `Quotation` table (a
  `/brain-gap` candidate against `policy-lifecycle.md`); no `apps/api` e2e.
- **Verification**: +34 api unit tests — `quotation.config.spec.ts` ×14
  ...
  scoping + chain grouping; get). `rfq.spec.ts` Playwright **8/8** (+2: capture a quote,
  revise into a new version; `@a11y` now also covers the Quotations section). Full suites
  green: **660** api unit (56 files, workspace-scoped), api e2e **98/98**, api contract
