...
### Part C · Domain D #31–39 — built, with these deferrals
...
- **Domains C–H** — Claims **#23–30 are built** (Notification, Registration, Documentation,
  ...
  reporting modules. **Finance (Domain D) — Premium Billing (#31), Collection (#32),
  Client Accounting (#33), Insurer Accounting (#34), Commission Calculation (#35),
  Commission Reconciliation (#36), Payment Processing (#38) and Bank
  Reconciliation (#39) are built**
  (#37 Refund Management needs no separate build — endorsement-driven `Refund` +
  maker/checker under #22 covers it). See the Domain D table above — the full
  `Invoice → Receipt → Reconciliation → Remittance` cycle through the workflow engine,
  with a client-funds ledger entry at each money movement, the on-the-fly
  ...
  VAT table, and the `CommissionLedgerEntry` ledger with the manual-override
  maker/checker, VAT snapshotting, `outstanding → paid` reconciliation against the
  insurer statement, `→ reversed` driven by a Process 22 cancellation, and the
  governed masked-only `PaymentChannel` list wired (optionally) onto every
  `Receipt` / `Remittance`, and the `ReconciliationException` variance job that
  ALWAYS raises an exception with the exact variance (never a silent write-off)
  plus its investigate / resolve path driving the `Invoice`
  `EXCEPTION_RAISED → EXCEPTION_RESOLVED → RECONCILED` hops);
