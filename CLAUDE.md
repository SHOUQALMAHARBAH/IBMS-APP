...
|------|--------|-----------------|
| 2026-09-07 | Part E opened — **Sales + Policy + Claims + Financial Dashboards** (backlog Process #64, "Executive Management Reporting — Part E below": Part E IS #64's own detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). **Sales**: `GET /dashboards/sales` — new leads/conversion rate, premium written (new vs. renewal, splitting on `Opportunity.isRenewal`), commission income, cross-sell/up-sell conversion — reuses the ALREADY-CONSUMED `dashboard.sales.view` permission. **Policy**: `GET /dashboards/policy` — active/expiring policies are a LIVE snapshot as of `now`; new-issued/cancelled are period-scoped. Cancelled policies scope by `Endorsement.appliedAt`, not `Cancellation.createdAt`. **Claims**: `GET /dashboards/claims` — open vs. closed claims, outstanding claims value, claims ageing, loss ratio by client/line/insurer. Every metric here is current-state, so unlike Sales/Policy there is NO period range at all — a single `asOf` reference-date override covers Part E's "filterable by time period" rule instead. Required WIDENING backlog #30's own `LOSS_RATIO_GROUP_BY` with an `insurer` dimension — a free capability upgrade to the pre-existing `GET /claims-analytics/loss-ratio?groupBy=` endpoint too. **Financial**: `GET /dashboards/financial` — receivables and ageing, payables to insurers, commission income and outstanding commission, profitability by client segment/line. **Neither a fresh build nor a pure "verify, don't build" outcome** — backlog #40's own `FinancialReportService.summary()` already computes every section verbatim, but its own DTO said outright "No line / insurer / branch filters here — those are a Part E dashboard refinement." Closed exactly that gap: reused #40's own pure builders via direct import, and widened THREE existing repositories (`InvoiceRepository`, `FinancialReportRepository`, the SHARED `ProfitabilityPolicyRepository` also used by #63) with additive optional branch/line/insurer scope params — no existing caller's call site changed, both #40's and #63's own e2e suites re-confirmed green. Deliberately did NOT widen `loadInsurerRemittances` (a `Remittance` has no `Policy` relation — a real "not applicable" filter case). `dashboard.financial.view`'s role grant is a strict subset of the pre-existing `financial-report.view`'s, so no practical access gap existed — kept its own dedicated route anyway since the filtering needs genuinely differ. No dashboard needed a migration or a cross-module SERVICE dependency (the #58 KPI Dashboard "read tables directly" shape, plus the #63 "share the repository" shape for Claims/Financial). **Verification**: +16 (Sales), +11 (Policy), +14 (Claims), +8 (Financial) api unit → api unit **2287** (183 files, from 2238). New e2e specs `sales-dashboard` 4/4, `policy-dashboard` 4/4, `claims-dashboard` 5/5, `financial-dashboard` 5/5 — permission gating, validation edge cases, full metric walks (Claims/Financial isolated via a fresh `Insurer` row since their scopes have no lower bound, unlike a period range), a Sales branch-scoping test, a Policy `renewalWindowDays`-override test. Full api unit suite 2287/2287 confirmed green; full 61-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`, plus transient TOTP-timing flakes re-confirmed clean in isolation. New Playwright specs 3/3 each (one real test-authoring bug caught and fixed in the Financial spec — a `getByText` strict-mode violation); full Playwright suite **276/276** (from 264) — one confirmed-transient Chromium crash on an unrelated a11y test, re-confirmed clean in isolation. `npm run typecheck`/`lint`/`build` (api + web) OK. | No migration, no seed change — `dashboard.sales.view`/`dashboard.policy.view`/`dashboard.claims.view`/`dashboard.financial.view` were all already pre-seeded. |
