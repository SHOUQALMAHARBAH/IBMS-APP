...
|------|--------|-----------------|
| 2026-09-07 | Part E opened — **Sales + Policy Dashboards** (backlog Process #64, "Executive Management Reporting — Part E below": Part E IS #64's own detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). **Sales**: `GET /dashboards/sales` — "new leads and conversion rate, premium written (new vs. renewal), commission income, cross-sell/up-sell opportunity conversion" — reuses the ALREADY-CONSUMED `dashboard.sales.view` permission (backlog #59's own `GET /sales-performance` uses the same code for a narrower quota-tracking metric; the two endpoints deliberately coexist). "Premium written (new vs. renewal)" splits on `Opportunity.isRenewal` — a real boolean already on the schema — via two `Policy.aggregate()` relation-filtered calls combined with `addMoney()`/`formatMoney()`, never by parsing formatted money strings back into JS floats. Filter applicability is real, not uniform: cross-sell/up-sell have no branch/insurer dimension at all (system-detected against a Customer, no officer-ownership concept). **Policy** (`dashboard.policy.view`'s first real consumer): `GET /dashboards/policy` — "active policies, expiring policies (renewal window), new policies issued, cancelled policies and cancellation reasons." Active/expiring are a LIVE snapshot as of `now`, ignoring the period filter; new-issued/cancelled are period-scoped — a deliberate asymmetry read off the backlog bullet's own mixed phrasing. Expiring reuses `RenewalCase.leadTimeDays`'s 90-day default, caller-overridable via `renewalWindowDays`. **Cancelled policies scope by `Endorsement.appliedAt`, not `Cancellation.createdAt`** — confirmed by reading `EndorsementService.apply()`'s cancellation branch directly, since `Policy.status -> CANCELLED` fires exactly when `appliedAt` is stamped. `reason` (free text) is a capped `findMany` list, never a `groupBy`. Unlike Sales, every Policy Dashboard metric reads the same `Policy` table, so branch/line/insurer filters apply uniformly with no per-metric carve-out. Neither dashboard needed a migration, a new permission, or a cross-module service dependency (the #58 KPI Dashboard "read tables directly" shape). **Verification**: +16 (Sales) then +11 (Policy) api unit → api unit **2265** (179 files, from 2238). New `test/sales-dashboard.e2e-spec.ts` 4/4 and `test/policy-dashboard.e2e-spec.ts` 4/4 — permission gating, 422 partial-period-overrides, full metric walks against distinctive historical period windows, a Sales branch-scoping test and a Policy `renewalWindowDays`-override test. Full api unit suite 2265/2265 confirmed green; full 59-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright specs 3/3 each; full Playwright suite **270/270** (from 264) — one confirmed-transient Chromium crash on an unrelated a11y test, re-confirmed clean in isolation. `npm run typecheck`/`lint`/`build` (api + web) OK. | No migration, no seed change — both `dashboard.sales.view` and `dashboard.policy.view` were already pre-seeded. |
