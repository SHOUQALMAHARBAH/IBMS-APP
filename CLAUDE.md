...
|------|--------|-----------------|
| 2026-09-07 | Part E opened — **Sales Dashboard** (backlog Process #64, "Executive Management Reporting — Part E below": Part E IS #64's own detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). New `GET /dashboards/sales` — "new leads and conversion rate, premium written (new vs. renewal), commission income, cross-sell/up-sell opportunity conversion" — reuses the ALREADY-CONSUMED `dashboard.sales.view` permission (backlog #59's own `GET /sales-performance` uses the same code for a narrower quota-tracking metric; the two endpoints deliberately coexist). **Filter applicability is real, not uniform** — "filterable by branch/line/insurer/period" is honored only where each dimension exists on the underlying model: cross-sell/up-sell have no branch/insurer dimension at all (system-detected against a Customer, no officer-ownership concept). **"Premium written (new vs. renewal)" splits on `Opportunity.isRenewal`** — a real boolean already on the schema, found by checking rather than assuming new modeling was needed — computed via two `Policy.aggregate()` relation-filtered calls combined with `addMoney()`/`formatMoney()`, never by parsing formatted money strings back into JS floating-point numbers. No migration, no new permission, no cross-module service dependency (the #58 KPI Dashboard "read tables directly" shape). **Verification**: +16 api unit → api unit **2254** (177 files, from 2238). New `test/sales-dashboard.e2e-spec.ts` **4/4** — permission gating; a 422 partial-period-override; a full leads/premium/commission/cross-sell/up-sell walk; a branch-scoping test proving cross-sell/up-sell stay unscoped. Full api unit suite 2254/2254 confirmed green; full 58-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `sales-dashboard.spec.ts` 3/3; full Playwright suite **267/267** (from 264). `npm run typecheck`/`lint`/`build` (api + web) OK. | No migration, no seed change — `dashboard.sales.view` was already pre-seeded. |
