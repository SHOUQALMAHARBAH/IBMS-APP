...
|------|--------|-----------------|
| 2026-09-07 | Part E opened — **Sales + Policy + Claims + Financial + Compliance Dashboards** (backlog Process #64, "Executive Management Reporting — Part E below": Part E IS #64's own detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). **Sales**: `GET /dashboards/sales` — new leads/conversion rate, premium written (new vs. renewal), commission income, cross-sell/up-sell conversion. **Policy**: `GET /dashboards/policy` — active/expiring policies (live snapshot); new-issued/cancelled (period-scoped, cancellation dated by `Endorsement.appliedAt`). **Claims**: `GET /dashboards/claims` — open/closed, outstanding value, ageing, loss ratio by client/line/insurer; NO period range (a single `asOf` instead); widened backlog #30's `LOSS_RATIO_GROUP_BY` with an insurer dimension. **Financial**: `GET /dashboards/financial` — receivables/payables/commission/profitability, closing the exact gap #40's own DTO deferred as "a Part E dashboard refinement," reusing #40's pure builders + widening 3 repositories. **Compliance**: `GET /dashboards/compliance` — KYC status, complaints by status/category, compliance breaches/exceptions (open AML alerts + the latest persisted Internal Controls self-approval scan, never a live re-scan), regulatory filing status, open DSRs, breach-register status, DPIA backlog — seven sections, each reading a different table directly, the DPO Workspace aggregate-screen shape. **NO period range AND `insuranceLine`/`insurerId` omitted dashboard-wide** — none of the seven registers ties to a Policy at all, the first Part E dashboard where two whole filter dimensions fail to apply anywhere; `branchId` is the only cross-cutting filter with real reach (KYC/Complaints/DSR/AML tie to a Customer, the other three don't). "Compliance breaches/exceptions" was a genuinely drafted interpretation (no named model) — chose AML alerts (Compliance-owned via `aml.monitor`) over `ReconciliationException` (Finance-owned) for the open-exception count, plus the most recent Internal Controls scan's violation count (read, not re-run, since a live scan is expensive). No dashboard needed a migration; only Financial needed to widen existing repositories. **Verification**: +16 (Sales), +11 (Policy), +14 (Claims), +8 (Financial), +16 (Compliance) api unit → api unit **2303** (185 files, from 2238). New e2e specs for all five, each permission-gated with a full metric walk (Claims/Financial isolated via a fresh Insurer row; Compliance via branch-scoping for 4 sections + BEFORE/AFTER deltas for the 3 that have no owner dimension, the DPO Workspace technique). Full api unit suite 2303/2303 confirmed green; full 62-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing — the system ran roughly 2x slower than earlier in this same work, so `rbac` needed a 180s re-run to confirm it was machine load, not a regression. New Playwright specs 3/3 each (one real test-authoring bug caught and fixed in the Financial spec — a `getByText` strict-mode violation); full Playwright suite **279/279** (from 264) — one confirmed-transient Chromium crash on an unrelated a11y test earlier in this same work, re-confirmed clean in isolation. `npm run typecheck`/`lint`/`build` (api + web) OK. | No migration, no seed change — all five `dashboard.*.view` permissions were already pre-seeded. |
