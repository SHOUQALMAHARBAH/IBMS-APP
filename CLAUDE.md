...
|------|--------|-----------------|
| 2026-09-07 | Part E opened — **Sales + Policy + Claims Dashboards** (backlog Process #64, "Executive Management Reporting — Part E below": Part E IS #64's own detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). **Sales**: `GET /dashboards/sales` — new leads/conversion rate, premium written (new vs. renewal, splitting on `Opportunity.isRenewal`), commission income, cross-sell/up-sell conversion — reuses the ALREADY-CONSUMED `dashboard.sales.view` permission (backlog #59's own `GET /sales-performance` uses the same code for a narrower quota-tracking metric). **Policy** (`dashboard.policy.view`'s first real consumer): `GET /dashboards/policy` — active/expiring policies are a LIVE snapshot as of `now`; new-issued/cancelled are period-scoped. Cancelled policies scope by `Endorsement.appliedAt`, not `Cancellation.createdAt` — confirmed by reading `EndorsementService.apply()` directly. **Claims** (`dashboard.claims.view`'s first real consumer): `GET /dashboards/claims` — "open vs. closed claims, outstanding claims value, claims ageing, loss ratio by client/line/insurer." Every one of THIS bullet's metrics is current-state, so unlike Sales/Policy there is NO period range at all — a single `asOf` reference-date override (the #33 AR ageing shape) covers Part E's "filterable by time period" rule instead. `CLOSED` is the only terminal `ClaimStatus`; outstanding value uses `Settlement.netSettlement` once known else `Claim.estimatedLoss`; ageing buckets days-open into `d0_30`/`d31_60`/`d61_90`/`d90_plus`. **Loss ratio by insurer required WIDENING backlog #30's own `LOSS_RATIO_GROUP_BY`** (`+ 'insurer'`) — a free capability upgrade to the pre-existing `GET /claims-analytics/loss-ratio?groupBy=` endpoint too, reused via direct import of the SAME pure `buildLossRatioBreakdown` function (`LossRatioRepository` itself shared the #63 Profitability Analysis way — independently provided, no module wiring). No dashboard needed a migration, a new permission, or a cross-module SERVICE dependency (the #58 KPI Dashboard "read tables directly" shape). **Verification**: +16 (Sales), +11 (Policy), +14 (Claims) api unit → api unit **2279** (181 files, from 2238). New e2e specs `sales-dashboard` 4/4, `policy-dashboard` 4/4, `claims-dashboard` 5/5 — permission gating, validation edge cases, full metric walks (Claims isolated via a fresh `Insurer` row since its `asOf` scope has no lower bound, unlike a period range), a Sales branch-scoping test, a Policy `renewalWindowDays`-override test. Full api unit suite 2279/2279 confirmed green; full 60-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`, plus transient TOTP-timing flakes (`prospect`, `sales-performance`) re-confirmed clean in isolation. New Playwright specs 3/3 each; full Playwright suite **273/273** (from 264) — one confirmed-transient Chromium crash on an unrelated a11y test, re-confirmed clean in isolation. `npm run typecheck`/`lint`/`build` (api + web) OK. | No migration, no seed change — `dashboard.sales.view`/`dashboard.policy.view`/`dashboard.claims.view` were all already pre-seeded. |
